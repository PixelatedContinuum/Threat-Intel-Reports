...
description: "Technical analysis of an active state-owned-insurer compromise: a Turkish-speaking operator weaponizes the OpenClaw AI agent platform into a custom analytics platform (ARPA) to harvest the victim organization's enterprise observability stack across four stolen sources (IBM Instana + SolarWinds Orion + Zabbix + VMware Aria), and authors Turkish-language insider-recruitment documentation to a named Windows AD user. UTA-2026-013 — first public attribution."
...
- **An active compromise of a state-owned Turkish corporate victim.** The primary victim is **the victim organization**, the consolidated national insurer 81.1% owned by Türkiye Wealth Fund (TVF). The operator has held sustained access to the victim's enterprise observability stack across **four stolen credential sources** (IBM Instana commercial APM + SolarWinds Orion + Zabbix + VMware Aria vCenter/ESXi) for at least 73 days, polling on a 5-minute cadence via five systemd daemons running on a single DigitalOcean Frankfurt VPS. The campaign was independently surfaced through open-directory monitoring; **no internal victim detection has been observed in the 70+ day window between initial third-party discovery and this report**.
- **Two publication-defining novel findings.** First: an **Observability-Tool Reverse Pipeline** — stolen credentials for four enterprise monitoring platforms cross-correlated into the operator's own analytics platform (ARPA Korelasyon Motoru) against a single named victim. Closest documented adjacent (UNC6395 OAuth-based CRM breach, 2025) is structurally distinct (one-time exfiltration vs. sustained 4-source ETL). Novelty maintained at the top of the MODERATE band after full prior-art review. Second: **third-party identification of an operator-recruited insider before victim-org internal detection** — operator-authored Turkish-language tunnel-setup documentation (eight Markdown files) addressed to a named insider at the victim organization (a Windows AD user), with operator-supplied SSH keys and a PuTTY session named `ARPA_Tunnel`. No 2024-2026 published case combines all four structural factors of this finding.
- **Attribution via UTA-2026-013 — no prior vendor coverage.** This report documents UTA-2026-013 with zero prior coverage across all reviewed threat-intelligence sources. Zero prior Trend Micro / Mandiant / CrowdStrike / Kaspersky / Hunt.io threat-actor catalog coverage exists at investigation date. Attribution holds at **high-MODERATE 78%** within the canonical MODERATE band (70–85%) on Turkish-speaking, Turkish-located, intra-Turkey single-thread operator profile. Real-world identity remains **INSUFFICIENT** — the public GitHub handle (`MehmetARPA`) is preserved as a behavioral IOC only; no real-name identification is supportable from current evidence and naming would risk damaging an innocent person who shares the handle's surname. **The MehmetARPA GitHub account was suspended by GitHub Trust & Safety on 2026-05-25** following disclosure; the associated repository is no longer accessible.
- **Five-axis Turkish geographic convergence is the strongest single-dimension attribution evidence in the parent campaign.** Turkish-language operator artifacts (eight tunnel-setup documents + dashboard self-branding) + Turkish-handle GitHub identity + Turkish self-branding (`ARPA Korelasyon Motoru` = "ARPA Correlation Engine") + Turkish state-owned victim + Turkish residential ISP source IP (TurkNet AS12735) captured in active operator session during late-evening local working hours. The residential-ISP source connection is DEFINITE Turkish-located; the absence of VPN / Tor masking is consistent with either OPSEC failure or with a non-professional individual operator.
- **Disposition outcome — one Tier-2 step complete; victim-coordination steps continue post-publication.** GitHub Trust & Safety suspended the operator's `MehmetARPA` account on 2026-05-25, removing the public repository from accessibility. Standard CISO-direct notification is not appropriate here because the recruited insider sits inside victim AD with operator-provided SSH keys and an unknown threat-vs-coercion intent classification; insider-in-chain risk requires General Counsel and TVF Audit Committee back-channel sequencing before CISO notification. The disclosure cascade structure is documented at the contact-category level in Section 14; specific contact persons and full evidence-handoff content are held in the offline FULL briefing for the victim's internal investigation team only. The remaining victim-coordination disclosure steps are tracked in Section 13 and continue post-publication; this public report carries no victim-identifying detail, so publication does not depend on cascade completion.

Over a 16-phase investigation beginning 2026-05-16, The Hunters Ledger documented the campaign as filesystem-grounded artifacts pulled directly from the operator's open-directory-exposed VPS — not as output-side measurements or downstream effects. The operator-built ARPA platform is a production-grade Python application stack (TimescaleDB + Neo4j + Redis + SQLite persistence; five systemd daemons; ~780 file artifacts; AI-augmented natural-language query interface) built on top of the OpenClaw AI agent framework. The operator's self-branded codename — `ARPA Korelasyon Motoru`, Turkish for "ARPA Correlation Engine" — appears in code docstrings, the public-facing dashboard footer, the GitHub repository name, and the systemd unit naming convention; the platform is not commodity tooling and is not derived from any known published source. The compromise represents the rare convergence of an operator-built custom analytics platform, a state-owned victim, four cross-correlated observability sources, an in-network insider with operator-deployed reverse-SSH tunnel, and an AI-augmented natural-language query interface over stolen APM data — every dimension of which is documented from primary-source filesystem evidence.

This report fills the gap that motivated the investigation: a **publicly-documented case of external operator + recruited insider where third-party detection precedes organizational internal detection**, paired with a **publicly-documented multi-source observability-tool reverse-pipeline cross-correlated against a single named state-owned-corporate victim**. Per project policy and the operational-sensitivity protocol described above, all victim-PII identifiers, the operator's precise residential IP, and the operator's GitHub repository URL are redacted from the public report body and retained in the offline FULL evidence briefing for victim coordination.

### What Was Found

- **A single operator running a multi-source observability harvester against a named state-owned victim.** The arsenal pulled from the operator's open-directory-exposed VPS at `209.38.205.158` (DigitalOcean Frankfurt, AS14061) includes: a 5-daemon production-grade Python ETL platform (`arpa-instana-api`, `arpa-daemon`, `arpa-continuous`, `arpa-autolearn`, `arpa-parallel` systemd units, all running on a 5-minute polling cadence), four data persistence backends (TimescaleDB for time-series + Neo4j for topology graph + Redis for cache + SQLite for collector state and AI assistant data), a victim-side PowerShell collector script designed for execution by an insider on a victim-organization Windows workstation, eight Turkish-language operator-authored Markdown setup documents instructing the named insider how to deploy a reverse-SSH tunnel and PuTTY saved session, AI/ML anomaly-detection modules over 1,859 victim hosts (Isolation Forest + DBSCAN + Local Outlier Factor + statistical methods), and an AI-augmented natural-language query interface (`ai_service.py` + `ai_assistant.db`) backed by Moonshot AI's Kimi LLM provider rather than a Western LLM (Anthropic / OpenAI / Google) — a deliberate Trust-and-Safety-evasion choice.
...
- **A partner-ecosystem cascade scope, not multiple separate compromises.** Six victim-organization ecosystem partner entities appear in the operator's unified topology data (Halkbank, ZiraatBank, VakifEmeklilik, SBM, TARSIM, DASK) plus one subsidiary ([victim subsidiary]). These are **partner-integration endpoints visible through the victim organization's own observability monitoring**, not separately compromised infrastructure. Selective deep-dive against the Halkbank application snapshot (~85 KB of Halkbank-specific Instana data within the broader 43-app collection) is consistent with Iran-sanctions geopolitical-leverage targeting interest. The partner entities are in **notification scope** under the disclosure cascade — not in compromise scope. This distinction is critical for downstream coverage to avoid mischaracterizing the campaign as a 7-victim compromise.

### Why This Threat Is Significant

Three reasons distinguish this case from existing public reporting on observability-platform abuse and on AI-augmented espionage:

**First, the Observability-Tool Reverse Pipeline TTP has no documented prior art at this scale and structure.** Public reporting on observability-platform abuse exists for individual incidents — SolarWinds Sunburst (which is structurally a supply-chain compromise against the vendor, not a stolen-credential reverse pipeline against the customer) and UNC6395 (which is a one-time CRM exfiltration via stolen OAuth tokens, not sustained ETL). No published case documents an operator stealing credentials for **four** enterprise observability platforms and cross-correlating them against a **single named victim** with a **production-grade ETL pipeline** sustained over 73+ days. The defender implication: monitoring platform credentials must be treated as crown-jewel-class secrets with rotation timelines comparable to domain admin tokens — not as low-priority service-account integration tokens. Detection coverage in the linked detection file targets the multi-platform cross-authentication pattern (`Suricata rule 2` + `Sigma rule 7`) at the SIEM correlation layer.

**Second, third-party detection of an operator-recruited insider before victim-org internal detection is structurally rare.** The 2024–2026 public corpus on insider threats contains many cases of internal-detection-first events (Rippling/Deel 2025, Coinbase 2025) where the victim organization surfaced the insider through their own monitoring. It contains no case combining all four structural factors of this finding: (a) external third-party passive open-directory scan surfaces the operator-to-insider artifacts, (b) single named state-owned-corporate victim, (c) named individual insider identifiable from operator-authored documentation, (d) 70+ day gap between third-party discovery and presumed victim-side awareness. The defender implication is twofold: (1) insider-recruitment artifacts can be surfaced in operator open-directory exposures and treated as primary-source attribution evidence; (2) victim-side detection of insider-recruitment artifacts authored externally is structurally hard because the artifacts never touch victim infrastructure during authoring — they touch it only at the point of insider-deployment-execution.

**Third, the operator's AI-augmented natural-language query interface over stolen monitoring data is a CANDIDATE novel TTP distinct from the AI-misuse patterns documented in the parent investigation.** Three other AI-offensive patterns appear in the parent campaign — AI-generated offensive code (Cases 1, 3), AI workflow orchestration (Case 8), and AI permission-allowlist customization (Case 4). This case adds a fourth pattern: **AI-augmented infrastructure reconnaissance using stolen APM credentials**. The architectural intent is documented in `ai_service.py` and `ai_assistant.db` (a Moonshot AI / Kimi-backed natural-language query interface over the unified-topology Instana event data). The operational state at investigation time is best characterized as broken / development-stage rather than production. This is classified as **CANDIDATE novel TTP at N=1**; cross-operator validation at N≥2 is required before upgrading to confirmed-novel status. The defender implication is forward-looking: even when the current operator's AI-reconnaissance interface is broken, the architectural pattern is in operator workflow inventory — defender preparation for the broader class of LLM-as-intelligence-analyst-over-stolen-victim-telemetry is warranted.

### Key Risk Factors

This is an **active, named-victim-confirmed, state-owned-victim, insider-in-chain** compromise with an operator-authored documentation trail still configured at capture time. The risk framing reflects what the campaign **enables and what it has currently configured** (operator-controlled 10-year JWT, stolen credentials across four observability platforms, in-network insider with operator-deployed reverse-SSH tunnel) rather than abstract capability claims.
...
<tr><td>Ecosystem Cascade Scope</td><td>7/10</td><td>Six ecosystem-partner entities (Halkbank, ZiraatBank, VakifEmeklilik, SBM, TARSIM, DASK) plus one subsidiary ([victim subsidiary]) appear in operator topology data via the victim organization integrations. These are partner-notification scope (not separate compromises) — but Halkbank-specific deep-dive activity in operator files is consistent with Iran-sanctions geopolitical-leverage targeting interest. Partner-notification scope is in the disclosure-cascade Tier-2 (post victim-coordination).</td></tr>
</tbody>
</table>

**Overall Campaign Risk Score: 9.5/10 — CRITICAL.** The campaign is rated CRITICAL based on the convergence of three concurrent CRITICAL-class factors: (1) an active stolen-credential compromise of a state-owned corporate victim with credentials that the victim has not rotated for 70+ days since third-party discovery; (2) an in-network insider with operator-authored documentation and operator-supplied SSH keys whose intent cannot be determined externally; (3) a publication-defining novel TTP set (Observability-Tool Reverse Pipeline + Insider Recruitment with operator-authored documentation + CANDIDATE AI-Augmented Reconnaissance) with no documented prior art at this combination. The threat level should be reassessed downward only after **all four stolen credential sources are rotated**, **the insider's account is segregated and forensically reviewed**, and **the operator VPS is taken down via DigitalOcean abuse coordination sequenced after USOM and victim notification**.

### Threat Actor Summary

This is a **single-operator** case at the operator-class profile level. The actor is tracked as **UTA-2026-013** *(an internal tracking label used by The Hunters Ledger — see Section 9)*. No prior public attribution to this operator exists across Trend Micro, Mandiant, CrowdStrike, Kaspersky, Hunt.io (infrastructure threat-intelligence platform) threat-actor catalog, MITRE ATT&CK groups, or VirusTotal (commercial threat-intelligence aggregation platform) threat-actor associations at investigation date. Zero prior vendor coverage has been identified; attribution details are documented in Section 9.
...
This compromise is not a one-off incident — it is a sustained, multi-source observability-data exfiltration campaign against a state-owned Turkish corporate victim with an in-network insider configured at capture time. The risk profile for any observability-platform customer is twofold: the **immediate-victim risk** (the victim organization's stolen-credential remediation timeline is actively running pending the disclosure cascade), and the **broader-class risk** for any organization with an observability stack of comparable scope (IBM Instana on OCP + SolarWinds Orion + Zabbix + VMware Aria) without source-IP allow-listing and admin-token-grade rotation policy on monitoring credentials.

### Understanding the Real-World Impact

The captured arsenal tells defenders what the operator does with successful credential theft. Five operational outcomes are observable in the captured artifacts:

1. **Sustained observability-data theft from a single victim, cross-correlated across four platforms.** The four stolen observability credential sources (Instana, SolarWinds Orion, Zabbix, VMware Aria) are ingested into the operator's TimescaleDB + Neo4j + Redis stack on a 5-minute polling cadence by five concurrent systemd daemons. The unified-topology graph spans 7,552 elements representing the victim's internal infrastructure at a granularity comparable to a privileged internal architecture diagram. The polling has been running for at least 73 days continuously.
2. **Selective deep-dive against a high-value ecosystem partner.** Within the 43-application Instana access set, the operator's filesystem contains an approximately 85 KB Halkbank-specific deep-dive snapshot — disproportionately large relative to the typical per-application snapshot size. Halkbank is the Turkish state-owned bank at the center of the Iran-sanctions-evasion case (US v. Halkbank, ongoing through 2024–2025); selective Halkbank focus is consistent with geopolitical-leverage targeting interest.
...
- **Phase 1: Stolen credential rotation across all four observability platforms.** All Instana API tokens, all SolarWinds Orion service-account credentials, all Zabbix admin credentials, and all VMware Aria credentials must be rotated. The four rotations must be done in parallel — sequential rotation gives the operator a window to re-establish access via the not-yet-rotated platform.
...
- **Phase 4: Full forensic enumeration of operator-accessed scope.** Reconstruct the operator's full access pattern across all four observability platforms (which API endpoints, which time windows, which data volumes), the unified-topology graph state (the operator's view of victim infrastructure), and the Halkbank-specific deep-dive scope (because of the geopolitical-leverage dimension). This phase requires direct access to each platform's audit log.
- **Phase 5: Partner-ecosystem notification.** Notify the six ecosystem partners (Halkbank, ZiraatBank, VakifEmeklilik, SBM, TARSIM, DASK) and one subsidiary ([victim subsidiary]) that they appear as integration endpoints in the operator-accessed topology data. Partner notification is in disclosure-cascade Tier-2 and is sequenced after victim-side rotation completion.
...
| Selective Halkbank Iran-sanctions intelligence collection via the victim organization integration | HIGH | The Halkbank-specific deep-dive within the operator's Instana data is structurally selective; geopolitical-leverage targeting of Halkbank is the most plausible explanation. The defender implication: Halkbank's exposure as an integration endpoint to the victim organization carries downstream geopolitical-intelligence risk independent of Halkbank's own security posture. |
...
| the victim organization General Counsel litigation against the operator following identity disclosure | MODERATE | TVF-portfolio companies have legal-action history when faced with documented external operator compromises. Civil action against a Turkish-resident identified operator is administratively feasible if real-world identity is confirmed. |
...
<tr><td>Primary motivation</td><td>State-aligned-or-political espionage targeting TVF financial sector intelligence</td><td>MODERATE — 780-file zero-monetization sweep formally rules out commercial sub-types; sub-types (a) state-aligned-loosely-controlled (~40%) and (c) political/factional (~35%) tied at MODERATE high-end</td></tr>
<tr><td>Target profile</td><td>State-owned Turkish corporate observability stack with ecosystem-partner integration visibility</td><td>DEFINITE — the victim organization TVF 81.1% state-owned + observable partner-integration endpoints in operator data</td></tr>
...
> **Executive Impact Summary:** The ARPA platform's eight capability surfaces (subsections 4.1 through 4.8 below) function together as an integrated intelligence-analyst pipeline operating over stolen victim telemetry. The most operationally significant capabilities for defender prioritization are: 4.2 (4-source Observability-Tool Reverse Pipeline novel TTP), 4.4 (insider-recruitment artifact with operator-authored Turkish-language setup documentation), and 4.6 (AI-Augmented Infrastructure Reconnaissance using stolen APM credentials, CANDIDATE novel TTP). Subsections 4.1, 4.3, 4.5, 4.7, and 4.8 provide architectural context for those three priority surfaces.
...
Captured event volume in sampled window: 8,649 events. Captured cluster identification: events span both `.trs.local` (production Active Directory domain) and `.trslinux.local` (Linux estate domain) and `.ve.local` (likely VMware-managed cluster domain) internal-domain spaces. Credential access likely via VMware Aria's API to vCenter; specific credential capture not in operator code as cleartext.
...
> **Analyst note:** This subsection covers the second publication-defining novel finding: an operator-recruited insider inside the victim's Active Directory environment, with operator-authored Turkish-language documentation instructing the insider how to deploy a reverse-SSH tunnel. The insider's specific Windows AD user identifier is suppressed from this public report per operational-sensitivity protocol; the full identifier is in the offline FULL evidence briefing for victim-coordination use only. The structural rarity of this finding is the four-factor combination: third-party detection precedes victim detection + single named state-owned-corporate victim + named individual insider identifiable from operator artifacts + 70+ day gap before presumed victim awareness.
...
3. Cross-source correlation logic in `correlation_v3.py` ("ARPA Korelasyon Motoru v3 - Temporal Focus") merges per-source host records via host-identifier matching (FQDN normalization across `.trs.local`, `.trslinux.local`, `.ve.local` domains) and writes the merged topology to Neo4j.
...
|---|---|---|
| Halkbank | `app_Halkbank.json` (~85 KB — disproportionately large for selective deep-dive) | Ecosystem-partner integration; selective Halkbank focus consistent with Iran-sanctions geopolitical-leverage targeting interest |
| ZiraatBank | Application snapshot present in operator data | Ecosystem-partner integration via the victim organization payment / banking flows |
| VakifEmeklilik | Application snapshot present in operator data | Ecosystem-partner integration (pension fund operator) |
| SBM (Sigorta Bilgi Merkezi) | Application snapshot present in operator data | Turkish insurance regulatory data integration |
| TARSIM (Tarım Sigortaları Havuzu) | Application snapshot present in operator data | Turkish agricultural insurance pool regulatory integration |
| DASK (Doğal Afet Sigortaları) | Application snapshot present in operator data | Turkish natural-disaster insurance regulatory integration |
| [victim subsidiary] | Application snapshot present in operator data | the victim organization subsidiary |

The Halkbank deep-dive size (~85 KB Halkbank-specific Instana snapshot, vs typical ~5–10 KB per other partner application snapshot) is the operationally interesting selective-focus signal.
...
**Halkbank Selective Targeting:** The disproportionate Halkbank deep-dive size is consistent with the operator having a specific intelligence-collection interest in Halkbank — the Iran-sanctions-evasion case (US v. Halkbank, ongoing through 2024–2025) is the most plausible geopolitical context. This is the strongest evidence supporting the state-aligned-or-political sub-type classification for the operator's motivation.
...
**T+1:00 to T+1:30** — Cross-source correlation merges per-source host records via FQDN normalization (`.trs.local`, `.trslinux.local`, `.ve.local` domains) and writes the merged topology to Neo4j. The unified-topology graph element count is updated.
...
The structured IOC feed includes a small number of victim-organization internal hostnames, internal IPs, and internal domain references (`.trs.local`, `.trslinux.local`, `.ve.local`). These are flagged in the feed as `DO NOT BLOCK / MONITOR (victim)` because they are the victim's own infrastructure — blocking them would harm the victim further. They are present in the feed to support defender hunt and forensic-correlation work, not for blocklist deployment.
...
| Turkish target | the victim organization — state-owned Turkish corporate insurer with 81.1% TVF state ownership + Halkbank ecosystem-partner with Iran-sanctions geopolitical-leverage interest |
...
- Halkbank-specific deep-dive snapshot disproportionately large within operator's 43-application Instana access set
...
- Halkbank selective deep-dive consistent with Iran-sanctions geopolitical-leverage targeting interest
...
This section explicitly identifies what cannot be assessed from available evidence and what closes each gap. Surfacing gaps is part of attribution discipline; concealing gaps damages report credibility.
...
| 2 | Türkiye Wealth Fund Audit Committee — parallel back-channel notification | PENDING | Parallel to USOM; bypasses victim-org-internal channels per insider-in-chain risk mitigation |
...
**Initial claim:** No 2024–2026 published case combines all four structural factors of this finding (third-party detection precedes victim detection + single named state-owned-corporate victim + named individual insider identifiable from operator artifacts + 70+ day gap before presumed victim awareness).
...
- The operator's 5-axis Turkish convergence indicates a Turkish-located operator with Turkish-state-sector targeting interest. Other state-owned Turkish corporates are plausibly in operator interest scope (MODERATE-confidence inference based on the 5-axis Turkish convergence pattern — Turkish operator location, Turkish language tradecraft, Turkish state-owned target selection, Turkish residential ISP, Turkish working-hours pattern — even though no additional victims are visible in the current operator-filesystem evidence).
