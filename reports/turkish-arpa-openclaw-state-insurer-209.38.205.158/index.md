...
- **An active compromise of a state-affiliated Turkish corporate victim.** The primary victim — referred to throughout as **the victim organization** — is a state-affiliated Turkish financial-sector organization. The operator has held sustained access to the victim's enterprise observability stack across **four stolen credential sources** (IBM Instana commercial APM + SolarWinds Orion + Zabbix + VMware Aria vCenter/ESXi) for at least 73 days, polling on a 5-minute cadence via five systemd daemons running on a single DigitalOcean Frankfurt VPS. The campaign was independently surfaced through open-directory monitoring; **no internal victim detection has been observed in the 70+ day window between initial third-party discovery and this report**.
