...
| JA4X | ~~`7022c563de38_7022c563de38_795797892f9c`~~ | ⛔ **Retracted 2026-08-21, do not hunt on this.** It does survive certificate regeneration, which is what the original note said, but it also matches **2,442,470 certificate observations in a two-day window**, spanning MikroTik customer-premises routers, Cisco IOS devices, generic `localhost` certificates and `trojan-server` proxies. It fingerprints the certificate-generation idiom, not the operator. Survivability without a base rate is not selectivity |
...
No host other than the receiver has ever presented that certificate across 365 days of public scan data. That is a clean negative on the question of whether a successor receiver already exists, and it comes with a stated limit rather than as a proof, since a successor with a regenerated certificate under a different common name, or one that public scanning does not reach, would not appear. The receiver was still serving that certificate at 2026-08-19, which is later than the development box went quiet, so absence of assistant use is not absence of access.

A later re-test tightened that negative rather than loosening it. Testing the certificate hash alone leaves the obvious hole that a redeployed receiver would generate a fresh certificate and a fresh hash. So the test was repeated on the common name by itself, which a regenerated certificate would still carry, and across 120 days it returned only the production receiver. The one other host presenting `CN=receiver` is an unrelated Windows RDP endpoint that happens to use the word as a machine name. The negative now survives the broader test, and the limit that remains is narrower than the one first stated, because a successor would now have to change its common name as well as its certificate, or sit somewhere public scanning does not reach.
...
> **Note on UTA identifiers:** "UTA" stands for Unattributed Threat Actor. UTA-2026-022 is a tracking label The Hunters Ledger assigns to an actor I have observed but cannot yet link to a publicly named threat group. Other vendors and feeds will not use this label; it is specific to this publication. If later evidence ties the activity to a named actor, I will retire the label and update the report.
...
One limit on that negative had to be stated rather than buried, and it has since been closed. At the time of writing, the Hunt.io threat-actor catalog check with its multi-vendor alias mapping, and the JARM and JA4X sibling pivots, **did not run at all**. The service was blocked server-wide by a Cloudflare managed challenge for the entire attribution stage, which is an access block rather than a quota failure, so it was a not-checked rather than a searched-and-not-found.

The service became reachable again and both were run on 2026-08-21. The catalog returned nothing, which moves the named-actor negative from two independent sources to three and leaves the answer where it was. The sibling pivot did not return a sibling either, but it did return something I had not expected, which is that the JA4X I had nominated as the best pivot for finding one is a fingerprint shared by millions of unrelated hosts. That correction is carried in Section 8 and in the indicator feed. The gap is closed; one of the two things it was holding turned out to be wrong.
...
The full machine-readable indicator set is published separately as a [JSON IOC feed](/ioc-feeds/opendirectory-157-180-101-47-ssh-mitm-parasitic-credential-theft-20260812-iocs.json), undefanged and ready for ingestion. It carries 38 SHA256 file hashes plus one MD5 of the receiver as actually deployed, the seven operator addresses in Section 9, four operator URLs, the receiver's TLS certificate hashes and JARM value (the JA4X is carried in the feed's non-indicator block with its retraction, not as something to match on), three server banners, the port set, and the host-side artifacts covering file paths, naming patterns, service and cron entries, process names, command-line patterns, log strings, log-line formats, database artifacts and HTTP headers.
...
The full rule set is published separately at [Detection Rules, SSH Interception and Parasitic Credential Theft](/hunting-detections/opendirectory-157-180-101-47-ssh-mitm-parasitic-credential-theft-20260812-detections/), carrying 6 YARA rules, 15 Sigma rule-documents across 10 YAML blocks, and 3 Suricata signatures. A reader counting Sigma headings will find 10 rather than 15, because two of those blocks each pair a correlation rule with the two base rules it counts, and every document in a group carries its own tier. By tier the whole set is 15 alerting-grade rules and 9 hunting rules, with the Sigma documents splitting 7 and 8, all 6 YARA rules alerting, and the Suricata signatures splitting 2 and 1. The coverage-gaps section is worth reading before deploying any of it, and everything below is the reasoning that shaped those rules.
...
The Hunt.io threat-actor catalog check and the JARM and JA4X pivots **did not run** at the time of writing, blocked server-wide by a Cloudflare challenge for the entire attribution stage. That was a not-checked rather than a negative, and it was the single most re-runnable gap in this report. **Closed 2026-08-21:** the catalog returned nothing, taking the named-actor negative to three independent sources, and the JA4X pivot retracted its own premise.
...
No successor receiver exists in 365 days of public certificate scan data, and a 2026-08-21 re-test on the certificate's common name alone, which a regenerated certificate would still carry, found none either across 120 days. The negative comes with its limit attached, because a successor that changed its common name as well as its certificate, or one that public scanning does not reach, would not appear at all.
