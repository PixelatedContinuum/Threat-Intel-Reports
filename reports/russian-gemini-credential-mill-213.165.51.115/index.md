...
- **Same operator, two independent vendors.** Trend Micro published independent coverage of this operator three days before this report's investigation window closed (2026-05-22; operator tracked as "bandcampro"). Cross-identification is DEFINITE via a five-point IOC match including all four operator IPs and the `@americanpatriotus` Telegram channel. The Hunters Ledger contribution is **six net-new findings beyond Trend Micro's coverage**: full source-code analysis of the `ai_sniper_brute.py` LLM credential mutator (verbatim prompt template), operator-built unauthenticated Python-stdlib C2 backend with in-place iterative development evidence (`/api/v1/get_results` endpoint called by client but not server-implemented), three AI Operator Handoff Documents (structurally distinct from the `GEMINI.md` jailbreak persistence Trend Micro covered), named-victim identification (the US dental-practice victim) with HC3 disclosure path, GCP project `[victim-named GCP project — redacted]` operator-named-after-victim signal, and multi-layered Cloudflare Tunnel topology including live victim-side RDP+SSH at capture time.
- **CRITICAL threat level driven by an active HIPAA-regulated US healthcare compromise.** The healthcare victim (US dental practice, AD domain `[victim AD domain — redacted]`) is the primary victim with full local NTLM hash inventory captured (6 hashes including an empty-password Administrator account at hash `31d6cfe0...089c0`), the OpenDental MySQL root hash captured, two internal subnets compromised, and operator-controlled persistent Cloudflare Tunnel access configured at capture time (`windows_server.tralalarkefe.com` for RDP/WinRM and `gil_dr1.tralalarkefe.com` for SSH). Patient Health Information (PHI) governed by HIPAA is at material risk pending tunnel teardown.
- **Three novel TTPs anchor the technical contribution.** **AI Operator Handoff Documents** — three exemplar Markdown files (`C2_MIGRATION_GUIDE.md`, `C2_INFRA_TRANSFER.md`, `DEPLOYED_TOOLS.md`) authored by the operator to prime new Gemini CLI sessions with operational context, with explicit AI-to-AI headers (`**To:** Gemini CLI` / `**From:** Gemini CLI`) and session-start load directives (`When starting a new session, refer to this file`). This artifact class is structurally distinct from Trend Micro's `GEMINI.md` jailbreak persistence and is HIGH-confidence first public documentation. **LLM-Personalized Credential Mutation** — Gemini 2.5 Flash generating 20 likely current password mutations per target from email + domain + known-password input via verbatim prompt `"Act as an expert red-team password analyst..."`. The operational pattern is independently confirmed by Trend Micro; this report contributes the first source-code analysis with verbatim prompt reproduction. **Operator-Built Unauthenticated Python-stdlib C2** — `c2_server.py` using `http.server.BaseHTTPServer` only (no FastAPI / Flask / aiohttp), four endpoints implemented (`/api/v1/update`, `/agents`, `/interact`, `/telemetry`), one called by client but not server-implemented (`/api/v1/get_results`) providing direct evidence of in-place iterative development against a live victim.
- **Vova75Rus-class operator pattern: Russian-native mid-tier with selective sophistication.** Four-axis profile from Phase 11 analysis of 122 Gemini CLI session JSONs: **Russian-native** (informal idiom `Бро` / `братух` / `Погнали` / `тачка` / `Комп Доктора` re-pasted across sessions; Cyrillic-English technical bilingualism; in-session credential ledger format string `Формат: Имя тачки - Юзер - Пароль`; DEFINITE confidence); **mid-tier selective sophistication** (sophisticated Cloudflare custom-domain Tunnel topology + GCP multi-instance overlay + AI workflow integration vs. rough OPSEC of plaintext Cloudflare full-admin API token + GitHub PAT in plaintext + 40+ Gemini API keys not encrypted + victim-named GCP project; HIGH selective confidence); **active campaign with concurrent R&D** (multiple Gemini CLI session threads, operator iterating on C2 source while deployed against the healthcare victim; HIGH confidence); **hybrid resource model** (custom-built C2 + commodity nuclei + paid AntiPublic.one subscription + 40+ stolen Gemini keys; HIGH confidence).
- **Cross-domain financial cybercrime + political IO from a single operator is rare in published reporting.** The operator runs the credential mill against US healthcare in parallel with a US-targeted Telegram disinformation channel `@americanpatriotus` ("Quantum Patriot" branding; ~17,000 subscribers per Trend Micro; 5-year campaign since 2021 with AI pivot September 2025). Disinfo posting workflow captured live in Gemini CLI session 2026-03-25T18-27 with anti-fraud / JD-Vance themed content authored by Gemini under operator instruction. DFRLab and OpenMinds document Russian-operated US-targeted Telegram networks at scale, but typically attribute these to coordinated teams rather than solo actors — this single-operator cross-domain combination is the notable behavioral observation.

The Hunters Ledger investigation independently surfaced this operator via Hunt.io open-directory monitoring of `213.165.51.115` (AEZA AS210644), then deep-pulled the operator's full arsenal across multiple investigation phases. Trend Micro's 2026-05-22 publication independently surfaced the same operator via a different investigative path; both investigations converge on a DEFINITE 5-point IOC match. This sub-report retains attribution at **MODERATE 83%** (top of the MODERATE band 70-85%, with Trend Micro Tier-2 corroboration; further details in Section 9). **No named-actor attribution is supportable** from current evidence — `bandcampro` is a Trend Micro vendor tracking handle and is not a real-name identification. This gap closes with the questions identified in Section 12.

This report fills the gap that motivated the investigation: **first publicly-documented complete operator-side AI-orchestrated credential mill** with operator-authored Markdown handoff documents written to prime new Gemini CLI sessions, source-code-level analysis of an LLM credential mutation pipeline with the verbatim prompt template, an incomplete-but-active operator-built C2 with direct iterative-development evidence, and a named-victim active HIPAA-regulated US healthcare compromise visible at the same moment as the operator-side capture. Per project policy, all credential strings in the report body are defanged to first-8 + last-4 characters; full strings remain in the offline evidence inventory and structured IOC feed.

### What Was Found

- **A single Russian-native operator running a multi-component AI-augmented credential mill.** The arsenal pulled from `213.165.51.115` includes: a custom Python stdlib HTTP C2 server (`c2_server.py`, 5 endpoints, unauthenticated), a Gemini-CLI-augmented per-target password mutator (`ai_sniper_brute.py` calling Gemini 2.5 Flash for 20 mutations per target), a stolen LLM API key validation pipeline (`check_keys.py` + companion `test_gemini_3.1.py` + `retest_keys.py` against an inventory of 40+ Gemini keys with MD5 hash tracking + 1 OpenAI key + 1 Venice AI key), a paid commercial breach-data integration (`mass_wp_mutator.py` calling `antipublic.one/api/v2/search`), and a Telegram disinformation posting automation (`quantum_patriot.py`).
- **A victim-named GCP project plus a four-node operator cloud overlay.** Operator-owned Google Cloud Platform project `[victim-named GCP project — redacted]` (operator named the project after the victim — direct evidence of dedicated focus rather than opportunistic compromise) plus existing project `elated-gizmo-491112-k0`, three GCP instances (Ghost Proxy `34.34.57.141` NL with all GCP logging explicitly disabled — deliberate OPSEC investment, Mailpit `35.192.41.201` US, Windows C2 `34.34.81.129` NL), one GCP service account email `geminicli@elated-gizmo-491112-k0.iam.gserviceaccount.com` linking all three.
- **A Cloudflare Tunnel topology under operator-owned custom domain.** Domain `tralalarkefe.com` with operator-owned Cloudflare zone (`6d415863...18f47af5`) and operator-captured full-admin Cloudflare API token (`pBkvccy9...TBztGF2`, defanged) fronting six tunnel subdomains: `c2.tralalarkefe.com` (C2 API), `payloads.tralalarkefe.com` (`.ps1` payload distribution), `windows_server.tralalarkefe.com` (victim-side RDP/WinRM), `gil_dr1.tralalarkefe.com` (victim-side SSH), `catchall1.tralalarkefe.com`, and `10101.tralalarkefe.com`. Plus one ephemeral bootstrap tunnel at `tenant-upcoming-great-descending.trycloudflare.com` for one-time payload bundle delivery that cannot be pre-blocked at the domain level.
- **Three AI Operator Handoff Documents.** Markdown files written by the operator and explicitly addressed to a Gemini CLI consumer. `C2_INFRA_TRANSFER.md` carries the literal `**To:** Gemini CLI` / `**From:** Gemini CLI` header and documents C2 endpoint topology for re-priming a new session. `DEPLOYED_TOOLS.md` carries the literal `When starting a new session, refer to this file` directive plus a tool inventory with paths and operational notes. `C2_MIGRATION_GUIDE.md` documents a planned C2 transport migration. None of these match the `GEMINI.md` jailbreak-persistence artifact class documented by Trend Micro — they are operational session-handoff documents, structurally distinct.
- **An active HIPAA-regulated US healthcare compromise with persistent operator access at capture time.** The healthcare victim (US dental practice, AD domain `[victim AD domain — redacted]`, primary server internal IP `[victim internal host — redacted]`, secondary host `[victim internal host — redacted]`, two compromised subnets) with full local NTLM hash inventory + OpenDental MySQL root password hash captured, and operator-controlled Cloudflare Tunnel access (`windows_server.tralalarkefe.com` for RDP/WinRM + `gil_dr1.tralalarkefe.com` for SSH) configured and operational at the moment of investigation. HC3 (HHS Health Sector Cybersecurity Coordination Center) is the disclosure coordination path; the healthcare victim direct practice notification is the parallel Tier-0 disclosure.

### Why This Threat Is Significant

This sub-report fills the gap between Trend Micro's 2026-05-22 macro-level "Patriot Bait" coverage and a defender-actionable artifact-level analysis. Trend Micro framed the operator's profile (Russian-speaking, AI-augmented, dual-track financial + influence operation) and provided the four operator IPs plus the `@americanpatriotus` Telegram channel — Tier-2 vendor evidence that anchors the cross-identification with The Hunters Ledger UTA-2026-012. This investigation extends Trend Micro's coverage with **six net-new contributions**:

1. **Source-code analysis of `ai_sniper_brute.py` with verbatim Gemini prompt reproduction.** Trend Micro confirmed the LLM credential mutation technique operationally; this report contributes the line-by-line prompt template, the per-target input structure (email + domain + known-password), the success-message format string (`[+++ AI SUPER GOOD +++]`), and the operator-self-named input/output file convention (`ULTRA_GOLD_TARGETS.txt` → `AI_SNIPER_GOODS.txt`). These are the highest-signal single-artifact YARA detection strings in the entire campaign.
2. **AI Operator Handoff Documents structured-handoff pattern (three exemplars).** The `C2_INFRA_TRANSFER.md` / `DEPLOYED_TOOLS.md` / `C2_MIGRATION_GUIDE.md` artifact class with the explicit `**To:** Gemini CLI` AI-to-AI header and `When starting a new session, refer to this file` session-start load directive is structurally distinct from the `GEMINI.md` jailbreak-persistence file that Trend Micro documented. AI Operator Handoff Documents represent a state-of-art tradecraft pattern: operator-written, AI-consumed, operational-context-bearing documents purpose-built to bootstrap a new AI session with prior session state. **HIGH-confidence novelty claim MAINTAINED**.
3. **Operator-built unauthenticated Python-stdlib C2 with iterative-development evidence.** `c2_server.py` is built on `http.server.BaseHTTPServer` only (no third-party web framework dependency), exposes four implemented endpoints (`/api/v1/update`, `/agents`, `/interact`, `/telemetry`) plus one called-but-not-implemented endpoint (`/api/v1/get_results`). The mismatch between client-side calls and server-side implementation is direct evidence of in-place iterative development against a live victim — the operator was deploying and iterating simultaneously. The C2 is unauthenticated by design — all five endpoints accept requests without credentials, an unauthenticated-takedown surface available to law enforcement and Cloudflare PSIRT acting under legal authority.
4. **Named-victim identification with HC3 disclosure path.** The US dental-practice victim identified through operator filesystem evidence (the GCP project `[victim-named GCP project — redacted]` operator-named-after-victim signal, plus the internal AD domain `[victim AD domain — redacted]`, plus the OpenDental MySQL root hash, plus the captured Cloudflare Tunnel inventory). HC3 (HHS Health Sector Cybersecurity Coordination Center) is the appropriate Tier-0 disclosure coordination path for HIPAA-regulated victims.
5. **Cloudflare full-admin API token + multi-layered tunnel inventory.** The operator's captured Cloudflare API token (defanged: `pBkvccy9...TBztGF2`) provides Cloudflare PSIRT a single subpoena-grade lever to tear down the entire C2 transport layer (all six tunnel subdomains + the operator-controlled zone). This is the single most actionable disclosure target in the campaign.
6. **GCP project `[victim-named GCP project — redacted]` operator-named-after-victim signal.** A named GCP project carrying the victim's name is a low-OPSEC tell that signals dedicated, not opportunistic, focus on this particular victim — and provides a billing-account-associated law enforcement identity anchor.

The combination of an active HIPAA-regulated US healthcare compromise with persistent operator access at capture time, three novel TTPs anchored in this case, and the operator-controlled Cloudflare full-admin API token + GitHub PAT + GCP service account identity surface gives defenders both a clear actionable disclosure path and a state-of-art reference for AI-augmented credential mill tradecraft.

### Key Risk Factors

This is an **active, actively-iterating, and named-victim-confirmed** credential mill campaign with co-located political IO. The risk framing reflects what the campaign **enables and what it has currently configured** (operator-controlled tunnels into a US healthcare victim, multi-instance C2, frontier-LLM-augmented attack pipeline) rather than what it does in absolute compromise-count terms.
...
**Overall Campaign Risk Score: 9.0/10 — CRITICAL.** The campaign is rated CRITICAL (not HIGH) because of the **active HIPAA-regulated US healthcare compromise with operator-controlled persistent tunnels configured at capture time**. The PHI breach timeline is actively running pending Cloudflare PSIRT tunnel teardown and the healthcare victim direct notification. If the tunnels are torn down and the victim notified, the threat level for residual capability would re-assess to HIGH (LLM-personalized mutation pipeline, frontier-LLM key inventory, and political IO track remain operational regardless of any single victim-side action).

### Threat Actor Summary

This is a single-operator case at the operator-class profile level. The actor is tracked as **UTA-2026-012** *(an internal tracking label used by The Hunters Ledger — see Section 9)* by The Hunters Ledger and as **bandcampro** by Trend Micro (TrendAI Research); both refer to the same operator (DEFINITE 5-point IOC cross-match). No named-actor attribution is supportable from current evidence — `bandcampro` is a vendor tracking handle, not a real-name identification.

- **UTA-2026-012 (= Trend Micro "bandcampro")** — MODERATE confidence (83%, top of the MODERATE band 70-85%, upgraded from parent MODERATE 75% via Trend Micro Tier-2 independent corroboration). Russian-native AI-augmented mid-tier criminal operator. Four-axis profile: Russian-native (DEFINITE via informal idiom register), mid-tier selective sophistication (HIGH), active campaign with concurrent R&D (HIGH), hybrid resource model (HIGH). Identity surface: GitHub handle `sonner1337` (with captured full-admin PAT defanged as `ghp_tdcX...DaRW`), Telegram channel `@americanpatriotus`, operator-persona-string `братух` in `GEMINI.md`, duty-free.cc Russian carding forum context. Real-world identity remains INSUFFICIENT (<50%); confidence ceiling without subpoena-grade disclosure or third Tier-2 vendor corroboration is HIGH 88-90%.
...
3. **For US healthcare and dental practice operators specifically: audit for `[victim AD domain — redacted]` AD domain artifacts and OpenDental installations.** This is the primary victim. Even if your environment is not the affected practice, the operator profile and TTP set apply directly to other small-practice US healthcare targets with similar attack surface (under-resourced security posture + commodity AD environment + OpenDental or similar industry-vertical practice management software).
...
This campaign is not a one-off attack — it is an active credential mill with a named US healthcare victim, persistent operator-controlled remote access into the victim environment, and a co-located political disinformation operation running concurrently. The risk profile for an organization is twofold: the **immediate-victim risk** (the healthcare victim has an active HIPAA breach timeline running pending tunnel teardown and direct notification), and the **broader-class risk** for any US small-practice healthcare organization with attack-surface profile similar to the healthcare victim.
...
4. **Frontier-LLM compute theft.** 40+ stolen Gemini API keys plus an OpenAI key plus a Venice AI key are inventory-tracked by MD5 hash and rotated through `check_keys.py` / `test_gemini_3.1.py` / `retest_keys.py` validation pipeline. This represents commercial AI-compute resource theft from legitimate key-holders — measurable cost shifted onto the legitimate billing accounts associated with each key.
...
**Why the hybrid resource model matters:** The operator does not build everything from scratch (which would be slow and expensive for a solo operator) and does not rely entirely on commodity tooling (which would limit the operator to the same throughput baseline as every other commodity-stuffing actor). The hybrid — custom C2 + LLM personalization + commodity breach data + commodity scanning — produces a measurably higher throughput per-operator-hour than either pure custom or pure commodity baselines. This is the AI-augmented mid-tier operator class in action: the operator extracts disproportionate operational leverage from frontier-LLM compute, which is currently the cheapest unit of intelligence the operator can buy (or steal).

---

## 5. Static Analysis

> **Analyst note:** This section walks through the operator's Python source code and Markdown handoff documents at the structural level. The captured arsenal is operator-built Python — there is no compiled binary stage for the credential mill components themselves (the only binary stage in scope is the PowerShell beacon `agent_final.ps1` (inferred Quasar-fork lineage, MODERATE confidence), which is documented from operator handoff reference but not directly extracted). The high-signal static-analysis findings are: (1) the verbatim Gemini role-priming prompt in `ai_sniper_brute.py`; (2) the three AI Operator Handoff Documents structural patterns; (3) the `c2_server.py` endpoint family and the `/api/v1/get_results` mismatch; (4) the operator's Russian-language comments embedded in source.
...
- **Track A — Credential mill against US healthcare:** Mass WordPress validation rig + AntiPublic.one breach-data lookups + LLM-personalized per-target mutation pipeline → the healthcare victim US healthcare HIPAA-regulated PHI compromise active at capture
