...
- **Hunt web-access logs for URI path component `/V5VgjLU0jsDe/`**: the 12-character random admin directory name is the single most BellaMain-specific indicator. Any HTTP request containing this path on a non-BellaMain host is a high-fidelity hit. See [Section 5.2](#usom-blocklist-self-monitoring-the-distinctive-turkish-targeting-tradecraft) and the [separate detection file](/hunting-detections/bellamain-turkish-phaas-79-137-192-3-20260516-detections/) for the Sigma rule.
...
The evidence rests on the open directory at `79.137.192.3`, which exposed `BellaMain.zip` (18.36 MB, SHA256 `f791fae4...`) containing the entire panel, 65 PHP files across 14 directories, listed in [Section 6.1](#61-file-inventory). Inside the archive sit the panel's authentication code (`signin.php`, `signup.php`, `logout.php`), the central AJAX dispatcher (`database/post.php`, roughly 350 lines), the live-victim-tracker widgets (`includes/girislog/*log.php`), the credential-submission handlers (`includes/forms/*.php`), the per-kit admin configuration forms (`includes/editforms/*.php`), and most importantly the obfuscated 12-character admin directory `V5VgjLU0jsDe/` holding `manager.php`, `backup.php`, `cekimbot.php` and `usmcheck.php`. The seven kit RARs and the CryptOne staging directory sit alongside the panel ZIP on the same open directory.
...
This is why the report is worth publishing standalone. It is the first public source-code disclosure of a Turkish PhaaS panel shipping these specific tradecraft features. Defenders previously had no visibility into the operator-side surface, and that surface is now documented.
...
The evidence is `BellaMain/V5VgjLU0jsDe/usmcheck.php`, which implements a one-shot poller against `https://www.usom.gov.tr/url-list.txt`. The core logic is straightforward, a `file_get_contents()` on the blocklist URL, then a `strpos()` of each entry against the panel's tracked kit domains (`dom_panel`, `dom_dolap`, `dom_letgo`, `dom_pttavm`, `dom_turkcell`, `dom_shopier`, `dom_yurtici`). On any hit the panel fires a Telegram alert to the operator announcement group `-1001817323952` with the message `"Usom Yedik Atış Stop"`, literally *"We got caught by USOM, stop the attack"*. The poller is invoked manually via the `/usom` Telegram command in `manager.php` rather than running on a schedule.

This matters because USOM is consumed downstream by Turkish ISPs as a DNS sinkhole input and by enterprise firewalls as a URL block list. A kit domain landing on the USOM blocklist sharply reduces its lifespan, since Turkish-resident victims become unable to resolve it. By polling USOM directly, the BellaMain operator gets early warning of any kit-domain listing and can switch to a fresh domain before the blocklist propagates downstream. They have wrapped Turkey's primary national cybersecurity control into a tool for *their own* OPSEC. That takes domestic knowledge to build, knowing USOM publishes a public URL list, knowing the polling URL, knowing which downstream consumers it feeds, and it is one of the strongest signals that the operator is Turkey-resident or has direct Turkey-domestic operational support. See [Section 9](#9-threat-actor-assessment).

For comparative context, no prior public PhaaS reporting documents a USOM-targeting blocklist monitor. The Cofense 2023 Telegram-credential-exfil patterns, the Sekoia EvilTokens 2024 PhaaS, the Netcraft Haozi 2025 PhaaS and the Breakglass TMoscow Bot 2025 PhaaS all lack any analogous national-CERT awareness. BellaMain's USOM monitor is a tradecraft innovation specific to this operator. I hold that HIGH on direct code observation, with no corroborating public reporting, and the negative result is itself the finding.

On the operator side, a PHP host on a non-Turkish-CERT-affiliated network making outbound HTTPS requests to `www.usom.gov.tr/url-list.txt` is a high-FP-context indicator, because legitimate Turkish security tooling, threat-intel platforms and individual researchers also fetch that URL. The high-confidence detection requires pairing the USOM fetch with another BellaMain artifact on the same host, the `sifreleWadanz` function-name pair, the `V5VgjLU0jsDe/` path, or the hardcoded canary Telegram bot URI. The Suricata signature is in the [separate detection file](/hunting-detections/bellamain-turkish-phaas-79-137-192-3-20260516-detections/).
...
The evidence is the `panel` table in BellaMain's MySQL schema, which stores four bot tokens per operator deployment, `adminbot_token`, `dekontbot_token`, `cekimbot_token` and `vergibot_token`. Each token drives a distinct Telegram bot with a specific role. Figure 2 lays out the four-bot architecture alongside the canary bot embedded in source.
...
The split between `dekontbot` for card data and `vergibot` for identity data is the operationally significant finding. In most documented phishing operations all victim data goes to a single channel and the operator monitors that channel. BellaMain *splits* the data streams, firing identity data immediately on Stage 1, separately from card data on Stage 2. That carries several implications:
...
For comparative context, Cofense (2023) documented single-bot, single-channel credential exfiltration, the dominant commodity pattern. Sekoia.io EvilTokens (2024) documented bot separation at the *PhaaS marketplace* level, covering sales, template deployment and anti-bot protections, but not at the per-deployment victim-data-routing level. Netcraft Haozi (2025) documented operator-community channel separation for FAQs and after-sales support, but again not per-deployment data-stream separation. Breakglass TMoscow Bot (2025) documented four-tier role-based access control in a web panel, but expressed those roles within a single panel UI rather than as distinct Telegram bot tokens. BellaMain's per-deployment four-bot architecture with identity-versus-card separation is more granular than any of these published patterns. I hold that MODERATE, because BellaMain's design is the only documented instance and the absence of prior reporting limits corroboration.
...
The evidence is `BellaMain/V5VgjLU0jsDe/manager.php`, which implements an admin Telegram bot with a 12-command vocabulary. Three of those commands are dedicated to data destruction:
...
Realistically this tradecraft is most effective against one specific scenario, law-enforcement seizure of the panel server with intent to recover victim data. It does *not* defeat network-traffic capture of stolen data in flight, Telegram-side legal process targeting the operator's account, recovery of `panel` MySQL backups that may exist on the hosting provider's snapshot system, or operator OPSEC failures, since the operator's own Telegram chat history is recoverable from their device on arrest. What it does ensure is that the live panel database is no longer a forensic prize at the moment of seizure, so defenders should plan evidence acquisition accordingly and favour live-system memory and network-traffic capture over post-seizure disk forensics.
...
The evidence is `BellaMain/V5VgjLU0jsDe/backup.php`, invoked when the operator sends `/yedek` to `adminbot`. The PHP source executes this sequence:
...
Two signatures are distinctive here:
...
The revenue flow is implemented across three files:
...
Detecting the revenue flow from the network layer is difficult, because the Binance API call is high-FP, the Telegram approvals are HTTPS, and the panel's MySQL writes are invisible to defenders without panel-host access. The strongest detection is on the panel-host process and source-file inventory:
...
The evidence is `BellaMain/database/fonk.php`, which defines two utility functions:
...
The function names are stable across samples. They are not parameterized, configurable, or generated, but literal strings hard-coded into the panel's core utility file. Any forked or rebranded BellaMain deployment reusing the same `database/fonk.php` would carry the same names, so a grep on `sifreleWadanz` across any PHP corpus, open-directory archive, or VirusTotal Intelligence sample set would surface every BellaMain-derivative panel by the same developer.

The pseudonym claim carries three caveats:
...
I hold this MODERATE. The function-name pair is a strong distinct-actor marker, since operators rarely sign panels with their handles and the pattern is consistent across every panel file observed. The named-actor pivot, resolving "Wadanz" to a person or a known underground handle, is INSUFFICIENT, because no public TI source surfaces this pseudonym and a cross-sample PHP corpus search returned zero hits. The expected upgrade path is paid-TI cross-reference (VirusTotal Intelligence, Recorded Future, Flare) or community recognition of the pseudonym after this publication.
...
The targeting logic is clear enough. The seven brands collectively cover Turkey's everyday consumer-transaction surface, selling on classifieds, postal shipping, cargo tracking, mobile bill payment, small-merchant checkout, second-hand fashion. A victim conditioned to receiving legitimate SMS and email notifications from any of these platforms is BellaMain's target persona. The lure-delivery vector, whether SMS, email, social media or search ad, is operator-configurable per campaign, and Stage 1 analysis recovered no specific delivery infrastructure beyond the kit-distribution server itself.

Lure fidelity is high. Each kit's `index.php` loads the impersonated platform's actual CDN assets (`https://cdn.dolap.com/web/css/bootstrap.min.css` for the Dolap kit, equivalent paths for the others) plus Google Tag Manager container `GTM-K7F5T5N` and New Relic RUM beacons (`https://js-agent.newrelic.com/nr-1026.min.js`). The combination produces a page loading the same fonts, the same CSS and the same client-side telemetry as the genuine platform. The only differences a victim might notice are the URL, on an operator-controlled domain, and the eventual payment-and-identity capture flow.

The code structure is stable across kits. All seven share the same `database/connect.php` MySQL credentials (`jakartaxdw` / `dbjakartaxdw` / `W!@25#8Tb2gxq15`), the same hardcoded canary Telegram bot token (`6797512084:AAGbJVoC...`), the same `girislog.php` anti-researcher canary, and the same `tgvergi.php` and `tgdekont.php` Telegram exfil patterns. The per-kit `kartayyil` field concatenation (`$kartay . " / " . $kartyil`), which preserves the literal " / " separator, suggests a single shared template that the kits were generated from with brand-specific HTML and CSS substitution.
...
Three connected mechanisms implement the gating:
...
At T+0:00 the lure is delivered, and that step sits out of scope here. A victim receives an SMS, email, social-media DM, or sponsored search ad carrying a link to a BellaMain kit domain. The lure-delivery vector is operator-configurable per campaign, and Stage 1 analysis recovered no specific lure-delivery infrastructure beyond the kit-distribution server.

At T+0:30 the kit page renders. The victim's browser loads the kit's `index.php`, for the Dolap kit a fake Dolap marketplace product listing. The page pulls legitimate Dolap CDN assets (`https://cdn.dolap.com/...`), the operator's Google Tag Manager container `GTM-K7F5T5N`, and embedded New Relic RUM beacons. Visual fidelity is high, and the page is visually indistinguishable from the real Dolap product page except for the URL.
...
At T+1:30 the victim enters credentials. They type a Dolap username and password on `login.php`, and the credentials INSERT into `hesaplar`. No Telegram alert fires at this stage, because the operator views captured credentials via the panel dashboard.

At T+2:00 the victim progresses through the funnel, led through `adres.php` for address, `ilan.php` for product confirmation, `odeme.php` for payment options, and `kartlaodeme.php` for the Stage 1 capture.

At T+3:30 comes the Stage 1 capture of identity data. The victim enters name, phone and Turkish national ID (TC Kimlik Numarası) on `kartlaodeme.php`, and `tgvergi.php` **immediately fires** via `vergibot_token`, sending the operator a Telegram message carrying the TC Kimlik Numarası, full name, phone, fake transaction amount, and date and time. This is the first Telegram alert in the funnel, so identity data fires *before* card data is even captured.

At T+5:00 comes the Stage 2 capture of the payment card. The victim continues to `ibanlaodeme.php` and enters PAN, expiry (MM/YYYY) and CVV, which INSERT into `kartlar`. Again no Telegram alert fires at this stage.

At T+6:30 the victim uploads the dekont. They are led to upload a fake bank statement, a "proof of payment", as a `dekont` image, which is stored in the kit's `dekontlar/` directory. `tgdekont.php` then fires a Telegram message to the operator via `dekontbot_token`, carrying inline `[Onayla]` (Approve) and `[Reddet]` (Reject) buttons alongside the victim's declared transaction amount.

At T+6:45 the operator approves. From their phone they click `[Onayla]` in Telegram, the webhook hits `tgdekont.php`, and the panel executes `UPDATE kullanicilar SET bakiye = bakiye + (amount * 0.7) WHERE id = <operator>;` plus a `toplamalinan` update. The operator's PhaaS-commission balance increases by 70 percent of the victim's declared amount.

At T+7:00 the victim is closed out. They see the `tamamlandi.php` "Transaction complete" page and are led to believe the payment went through. By then the operator holds their full identity, card and bank-statement data.

A parallel anti-researcher branch runs alongside this. If the victim's request URL contains the `?lg=` GET parameter, the kit short-circuits before any heartbeat, firing a Telegram alert to the canary group `-1002104835510` with the IP, user agent and `?lg=` value, then returning a plaintext Turkish-profanity troll response naming `@AresRS34`. Researchers and automated phishing-detection sandboxes probing the kit with the conventional `?lg=` URL-canary parameter will trigger it.
...
*Only the canary bot token (above, REVOKED) is recoverable from source. Operator-configured bot tokens for `adminbot`/`dekontbot`/`cekimbot`/`vergibot` live in the panel MySQL `panel` table and would surface only via DB compromise or honeypot acquisition, see [§5.3](#four-bot-telegram-c2-with-role-separation) for the structural detection approach.*

The panel does **not** beacon on a fixed interval, it is event-driven, with no schedule. Each credential or card capture triggers an immediate Telegram outbound call. The closest thing to beaconing is the live victim heartbeat on a 3-second AJAX cycle, but that runs **from the victim's browser to the panel** rather than outbound from the panel. There is no DGA, no fast-flux, and no beacon-interval signature to detect.

All operator-side C2 and exfil traffic uses TLS over standard HTTPS to public APIs, Telegram, Binance and USOM. The Telegram payloads themselves are unencrypted JSON message bodies, since Telegram terminates TLS and stores the content in its own database. There is no custom encryption layer beyond standard HTTPS.

PHP `file_get_contents()` and cURL calls use default user agents unless explicitly set, and no custom UA appears in the panel code, so user agent is not a useful detection signature here.
...
Checking tactic coverage, Reconnaissance, Privilege Escalation and Lateral Movement do not apply in this campaign, since the view is operator-side, pre-victim activity is not directly visible from panel code, the panel runs as the web-app user, and the architecture is single-server. The covered tactics, Resource Development, Initial Access, Execution, Persistence, Defense Evasion, Credential Access, Discovery, Collection, C2, Exfiltration and Impact, represent the full BellaMain capability surface.
...
The contextual fingerprints are Turkish-language consumer-fraud targeting across 7 named brands, USOM self-monitoring that implies knowledge unusual for a non-Turkey-resident operator, idiomatic Turkish profanity in the anti-researcher canary (`Sazan IP`, `Sazan Kod`, `Sazan Cihaz`), and a TRX/TRON 70/30 payout split priced off the live Binance TRXTRY rate.

The infrastructure fingerprints are Aeza continuity across more than 2 years (AS204603 in 2023 moving to AS216246 from 2024), Cluster A domains that are all attacker-controlled rather than compromised, and Turkish-market naming (`evotoptan.com`) at the domain-registration level.
...
- **`https://www.usom.gov.tr/url-list.txt`**: Legitimate Turkish security tooling, threat-intel platforms, and individual researchers fetch this URL. Detection signatures on this fetch must be combined with another BellaMain artifact on the same host (see [Section 5.2](#usom-blocklist-self-monitoring-the-distinctive-turkish-targeting-tradecraft)).
...
On scope, all rules cover Cluster A, the BellaMain Turkish PhaaS panel plus the 7 brand-impersonation kits, and nothing else. Cluster B (Inkognito) and Cluster C (Rhadamanthys) are covered by previously published detection sets. The evidence base is recovered PHP source code rather than PE binaries, so the detection logic targets server-side artifacts, web-server logs and network egress rather than endpoint process injection or PE characteristics.
...
H3 holds that the BellaMain operator is not Turkey-resident but a non-Turkish cybercrime team, most plausibly Russian given the Aeza continuity and bulletproof-hosting selection patterns, that has built a Turkey-targeting PhaaS using contracted Turkish-language assistance or LLM-assisted translation. Idiomatic Turkish, USOM awareness and TC Kimlik targeting are all operationally achievable from outside Turkey by a sufficiently motivated and resourced team.
