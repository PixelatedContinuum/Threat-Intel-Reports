...
The defensive bottom line is that the custom MSSQL CLR backdoor is the highest-value detection target in this campaign, because automated tooling will not catch it. VirusTotal sandboxing rates it harmless at 98 percent confidence, and Microsoft and Kaspersky both score it clean, since a host-process-dependent backdoor that only activates inside SQL Server is invisible to generic sandboxes and most signature engines. The deployable detection package accompanying this report, a YARA banner anchor, Sigma rules for `sqlservr.exe` spawning `cmd.exe` and for the MSSQL CLR install sequence, IIS webshell behavioral rules, native-tool imphashes, and a low-false-positive Flask C2 `/health` network signature, fills that gap. Any MSSQL, IIS or Active Directory environment is in scope, and the January-2026 Windows Error Reporting patch should be applied where it is missing.
...
| EfsPotato, GodPotato, SweetPotato, Rubeus, SharpSuccessor | Public tools, **operator-recompiled** | Identified by a YARA HKTL-GUID match and VirusTotal detections, plus a capability-signature match, not by public-release hash identity |
...
| **Family confidence** | Tool identifications **HIGH/DEFINITE** (VT hash + YARA + a capability-signature match). `cmd_exec.dll` = MSSQL CLR reverse-shell backdoor **DEFINITE** (decompilation). `CVE-2026-20817_PoC.exe` = non-weaponized demo PoC **DEFINITE** (disassembly). |
...
This means the backdoor turns the database server itself into a command-execution platform. Because the code runs as the SQL Server service account, and that account holds SeImpersonatePrivilege by default, a successful install lands the operator in exactly the privilege context the co-located Potato suite escalates from. `cmd_exec.dll` is functionally the MSSQL twin of the IIS webshells, a second independent way into the same SYSTEM-escalation chain.
...
<figure style="text-align: center; margin: 2em 0;">
  ...
  <figcaption><em>Figure 2: decompiler view of cmd_exec.dll. Top, the SQL-CLR <code>reverse_shell</code> method: a <code>TcpClient</code> loop that writes the <code>[*] Connected to SQL Server CLR backdoor</code> banner, reads operator commands, and exits on a null line or <code>"exit"</code>. Bottom, <code>ExecuteCommand</code> running <code>cmd.exe /c</code> with redirected output and a hidden window. The banner string is the report's strongest YARA anchor.</em></figcaption>
</figure>
...
The transferable lesson is that a host-process-dependent backdoor which only activates inside a specific service looks benign to every generic sandbox and most signature engines. Defending against this class needs either a string or structure anchor that fires on the file at rest, the YARA approach, or behavioral telemetry from the host process itself, `sqlservr.exe` spawning `cmd.exe` or initiating outbound TCP. Both are provided in the [detection package]({{ page.detection_page }}), and the `[*] Connected to SQL Server CLR backdoor` banner is the strongest single anchor because it is operator-specific and absent from any public reference implementation.
...
To frame this honestly, and I maintain the framing, I do **not** claim the operator holds a working CVE-2026-20817 exploit, and I do **not** call this build the public PoC. Whether it derives from the oxfemale release is unconfirmed, since no hash comparison was performed, and ours is non-weaponized while the public PoC is described as working, so they may differ. The accurate statement is that the operator carries a *non-weaponized demonstration PoC* for a recently-patched LPE, which makes this a **patch-action callout** rather than a live exploit threat. The operational read is that the operator relies on the proven commodity Potato escalators and experiments with newer LPE research on the side.
...
Checking tactic coverage, Initial Access, Execution, Persistence, Privilege Escalation, Credential Access, Defense Evasion, Discovery, Command and Control, and Lateral Movement are all represented. Collection, Exfiltration and Impact are **not** mapped, because no collection, staging or destructive capability is present in the sample set. This is a privilege-escalation and lateral-movement staging kit, not a data-theft or ransomware payload.
...
Prior art is credited to Breakglass Intelligence, who first documented the C2 panel on 2026-04-09 and likewise reported attribution as unknown [Source: intel.breakglass.tech post #158873].
...
Overall campaign risk is 6.2 out of 10, which is MEDIUM. The capability dimensions are uniformly high at 6 to 8, but the campaign-activity dimension at 2 out of 10, an idle C2, no victims and a single blockable host, governs the overall rating per the threat-level override at the top of this report. Were the C2 to activate or victims to be confirmed, the activity score would rise sharply and the overall rating would move to HIGH.
...
- **HIGH:** all public-tool identifications (VT + YARA + capability-signature matches); the host and network IOCs; the dual-vector foothold model; no sibling infrastructure (five convergent negative pivots); the Flask C2 three-route surface, in-handler auth, and vocabulary fingerprint.
