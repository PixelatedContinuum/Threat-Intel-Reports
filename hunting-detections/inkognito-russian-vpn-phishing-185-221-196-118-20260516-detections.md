...
| Sigma | 0 | 1 | T1566.002, T1684.001, T1036.005 | 5 |
| Suricata | 0 | 2 | T1071.001, T1583.004 | 6 |

> **Detection vs Hunting:** *Detection rules* are high-fidelity and evasion-resilient, safe to alert on. *Hunting rules* are broader, for scoping and threat-hunting. Expect to review the hits.

> ⛔ **Retraction, 2026-08-21.** Two rules originally published in this file, one Sigma and one Suricata, both keyed on the `kittenx` 404 decommission tombstone, have been withdrawn. Base-rate testing showed the tombstone is VKontakte's default server banner rather than an operator artifact. Both are struck through in place below, and both are excluded from the rule counts in the table above. Do not deploy them, and remove them if you already have.

**Highest-confidence anchors:**
- The wildcarded `*.inklens.ru` brand-impersonation subdomain query, re-anchored from a static 25-subdomain list to the parent zone so it now covers the operator's full (467+) and still-growing brand-impersonation inventory rather than only the subset enumerated during this investigation (Sigma Hunting, retiered 2026-09-09: the rewrite closed the enumerated-list weakness but the rule still anchors on nothing but the domain literal itself, which is Hunting per the tiering rubric, not Detection).
...
- **Suricata: `SIG_Inkognito_TLS_SNI_Operator_Domains`** (5 rules, SIDs 9001006 to 9001010). Each was a bare single-domain TLS SNI match (inkconnect.ru, inklens.ru, inklens.co.uk, cryptone.bot, unloki.ru) with no additional qualifier. All 5 domains are already BLOCK-listed in the feed.

**Cut: dead-code Suricata rule (1 rule, SID 9001001: not an atomic).** The original `SIG_Inkognito_KittenX_Tombstone_Response` section paired two alert rules intended to work together via `flowbits`: SID 9001001 matched any outbound HTTP GET request and set `flowbits:inkognito.kittenx.request` with `flowbits:noalert` (so it never itself alerts), apparently intended to gate SID 9001002's response check. However, SID 9001002 never checks `flowbits:isset,inkognito.kittenx.request`; it alerts independently on the full response signature. SID 9001001 as published therefore matched essentially all HTTP GET traffic, set a flag nothing reads, and alerted on nothing: zero detection value, so it was retired rather than kept as a broken rule. SID 9001002, the tombstone-response rule this one was meant to gate, survived that backfill as a Detection-tier rule, but it was itself withdrawn on 2026-08-21 after base-rate testing showed the `kittenx` tombstone is VKontakte's default server banner rather than an operator artifact. Neither SID is deployable. The full measurement is in the retracted Suricata rule block above.
