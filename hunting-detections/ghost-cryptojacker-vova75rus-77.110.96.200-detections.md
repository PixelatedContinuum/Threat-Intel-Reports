...
**False Positives:** Any legitimate or unrelated service that happens to use UDP 14433–14444 (uncommon but not impossible); the `threshold` limits alert volume per source. As of rev 3 the rule is scoped to egress, so internal-to-internal traffic on these ports no longer matches.
