...
**False Positives:** Any legitimate or unrelated service that happens to use UDP 14433–14444 (uncommon but not impossible); the `threshold` limits alert volume per source. As of rev 3 the rule is scoped to egress, so internal-to-internal traffic on these ports no longer matches. A specific, non-obvious case observed in production: a client on a road-warrior or full-tunnel VPN (any protocol) reaching its own gateway's dynamically-assigned public WAN address over a port in this range is scoped as EXTERNAL_NET traffic by Suricata's own address-group model, even though the traffic never leaves the LAN. This is a hairpin-NAT artifact, not genuine egress. There is no reliable content or flow-state anchor available to exclude it at the rule level; see the 2026-09-07 FP remediation note below for the measurement and the `flow:established` approach that was tested and rejected. Operators should add a local exception scoped to their own current WAN address and/or VPN client subnet if this fires.
**Deployment:** Network IDS/IPS at perimeter, server egress monitoring; hunt-tune before alerting, and corroborate with the AEZA-range Detection rule above before escalating. Confirm `EXTERNAL_NET` is defined as `!$HOME_NET` on your sensor rather than the literal `any`, or the egress scoping below has no effect.

**FP remediation note (2026-08-20):** two authoring defects, fixed together in rev 3.

The `msg` claimed "Backdoor Egress", meaning outbound to the internet, while the rule matched `$HOME_NET any -> any 14433:14444`. Suricata's `any` is an unrestricted wildcard that ignores the `$HOME_NET`/`$EXTERNAL_NET` partition, so internal-to-internal traffic matched a rule describing egress. That is what fired in production: one internal host to another, part of a UDP spray across 347 distinct destination ports of which 99.3 percent were dropped at the firewall, with port 14444 catching 24 events purely by coincidence of range. No tunnel, no cryptojacking. Scoping the destination to `$EXTERNAL_NET` costs nothing, because a Hysteria tunnel to operator infrastructure is external by architecture and there is no legitimate internal-to-internal case to preserve.

The second defect is the classification. This rule was already tiered Hunting/Robustness 2 above, and its own rationale concedes that "no content/host anchor is possible since this is opaque QUIC traffic". Despite that it shipped as `classtype:trojan-activity`, which reads to a subscriber exactly like the genuinely high-confidence AEZA-range Detection rule earlier in this file, with no machine-readable tier signal to distinguish them. The sibling Telegram-egress hunting rule in this same file correctly uses `classtype:policy-violation`; this rule was the outlier. Rev 3 aligns it and adds `signature_severity Minor`, so the tiering judgment already made upstream now survives into the artifact an analyst actually receives.

A port range with no behavioural anchor cannot reach Detection tier and should not be expected to. The port pair is, in this file's own words, "this operator's specific configuration choice, not a kit-wide structural constant", and it is trivially changed. The stronger signal already documented in the campaign IOC feed — multi-port egress combining TCP 3333 and 4444 with UDP 14433/14444 and contact with the kit's mining-pool domain inside five minutes — is a genuine chokepoint and would justify Detection tier, but it is a correlation rule that does not exist yet rather than an edit to this one. Until it does, treat a hit here as a lead requiring corroboration.

**FP remediation note (2026-09-07):** measured, not re-tuned. Every historical alert for this signature was pulled and accounted for. There are 28 emitted alerts across this signature's entire operational life to date (authored 2026-08-07, first fired 2026-08-18), cross-validated against three separate indices: the raw `logs-suricata-*` data stream, its concrete backing index `.ds-logs-suricata-default-*`, and the long-retention `.alerts-security.alerts-default` alert copy, which independently covers the rule's whole life with no ILM delete phase and shows the identical count. Zero of the 28 carry any corroborating GHOST, Hysteria v2, mining-pool, or other malicious signal. One is the already-documented pre-rev-3 internal-to-internal defect above. The remaining 27 trace to exactly two benign local causes, a road-warrior VPN client hairpinning to its own network's WAN address, and a local IoT camera device's cloud-relay session landing on a port inside this range, both handled by local network exceptions rather than a rule change.

A `flow:established` gate was tested as a possible content-free anchor, since the VPN-hairpin instance was fully one-directional (no reply packets seen). It was rejected. The camera-relay instance is also one-directional in its own flow record, while a second flow to the identical destination inside the same already-confirmed-benign event showed one reply packet, so bidirectionality is inconsistent even within a single benign case, and the gate would behave unpredictably rather than reliably separating benign from malicious traffic.
