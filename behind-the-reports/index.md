...
## A Plain-Language Guide for Anyone Curious About My AI-Assisted Analysis
...
Each agent operates not just on instructions but against a defined skill framework — a structured set of standards, methodologies, and domain knowledge built specifically for this workflow. These skills enforce consistency: the detection engineer doesn't just write rules, it writes rules to the exact submission standards required by the public repositories where they'll be shared. The attribution analyst doesn't just assess threat actors, it applies a structured evidence-weighting methodology that prevents overconfident claims. Every agent has guardrails built in at the knowledge level, not just the instruction level.
...
The agents are the visible layer of this workflow. Beneath them is a second layer of infrastructure — hooks and commands — that runs automatically, without instruction, to keep the workflow accurate, efficient, and resilient. This layer is what makes the whole system behave reliably rather than just approximately.

**Hooks** are automated scripts that fire on specific events — when an agent starts, when it finishes, when the reviewer completes. They handle everything that would otherwise require manual coordination:

- **Workflow State Tracker** — maintains a live JSON record of every agent that has run, in what order, and at what time. At any point in the workflow, the orchestrator can read this file to know exactly where things stand without having to ask or infer.
- **Dispatch Tracker** — records the precise launch timestamp of every agent. This enables accurate measurement of whether parallel agents actually launched simultaneously, and how long each stage took — data that drives future optimization decisions.
- **Quality Gate Checker** — fires automatically when the report reviewer completes. It reads the reviewer's quality score, compares it against the 8.0 threshold, checks for critical issues, tracks how many revision cycles have occurred, and writes a `REVISION_NEEDED` or `COMPLETE` decision back to the state file. The orchestrator doesn't decide this — the hook does, based on defined criteria, every time, without exception.
- **Result Cache Manager** — when the research analyst finishes, this hook extracts the malware family from the analysis, saves the research results to a family-specific cache file, and updates the state file. On future analyses of the same family, the orchestrator checks this cache before dispatching the research analyst at all. If valid cached results exist, the agent is skipped entirely — saving 15–20 minutes of work per re-analysis.
- **Skill Validation Gate** — fires after each agent completes and checks the state file to confirm the agent recorded usage of its required skill framework. If the malware analyst finishes without having invoked the MITRE ATT&CK mapping skill, or the detection engineer finishes without YARA and Sigma formatting validation, a warning is written to the state file and flagged for the quality gate. Standards compliance is enforced automatically, not assumed.
- **Token Usage Tracker** — logs input, output, and total token consumption for every agent into a structured metrics file. Each agent maps to a named workflow stage, so the metrics file shows not just total usage but cost per stage — useful for identifying which parts of the pipeline are most expensive and where optimization would have the most impact.
- **Session Milestone Saver** — at each major stage completion (malware analyst, parallel batch, attribution, report writer, reviewer, editor), this hook automatically backs up the workflow state to a timestamped session file. If anything interrupts the workflow mid-run, the last known good state is preserved and recoverable.

**Commands** are scripts invoked by the orchestrator at specific decision points:

- **Parse Checkpoint Feedback** — when you type feedback at Checkpoint 1 or Checkpoint 2, the orchestrator doesn't try to interpret your natural language itself. It passes your exact words to this script, which parses them into a structured JSON decision: proceed, re-run a specific agent, request major revision, or ask for clarification. The routing logic is deterministic and consistent regardless of how the feedback is phrased.

This supporting layer is not incidental. Designing and building it required understanding not just what the agents need to do, but what could go wrong, what needed to be measured, what standards needed enforcement, and how to make a multi-agent system that behaves predictably under real conditions. The agents do the analytical work. The hooks and commands make that work trustworthy.
...
**Cache check:** Is there already research on this malware family from a recent analysis? The Result Cache Manager hook saves research results by malware family after every run. If results for this family exist, the research analyst is skipped entirely — saving 15–20 minutes of work.
...
When you approve, the orchestrator reads the quality gate decision already written to the workflow state file. The Quality Gate Checker hook computed this automatically when the reviewer finished — the orchestrator doesn't make this judgment, it reads a decision that was already made against defined criteria. The reviewer scored the report across three expert perspectives:
